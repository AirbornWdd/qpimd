/* RIPd and kroute interface.
 * Copyright (C) 1997, 1999 Kunihiro Ishiguro <kunihiro@kroute.org>
 *
...

/* RIPd to kroute command interface. */
void
rip_kroute_ipv4_add (struct prefix_ipv4 *p, struct in_addr *nexthop, 
		    u_int32_t metric, u_char distance)
{
  struct zapi_ipv4 api;

  if (zclient->redist[KROUTE_ROUTE_RIP])
    {
      api.type = KROUTE_ROUTE_RIP;
      api.flags = 0;
      api.message = 0;
      ...
      if (distance && distance != KROUTE_RIP_DISTANCE_DEFAULT)
	{
...
void
rip_kroute_ipv4_delete (struct prefix_ipv4 *p, struct in_addr *nexthop, 
		       u_int32_t metric)
{
  struct zapi_ipv4 api;
...
static int
rip_kroute_read_ipv4 (int command, struct zclient *zclient, kroute_size_t length)
{
  struct stream *s;
  struct zapi_ipv4 api;
...
{
  zclient->redist[KROUTE_ROUTE_RIP] = 1;
  return CMD_SUCCESS;
}
...
DEFUN (no_rip_redistribute_rip,
       ...
       "Routing Information Protocol (RIP)\n")
{
  zclient->redist[KROUTE_ROUTE_RIP] = 0;
  return CMD_SUCCESS;
}

DEFUN (rip_redistribute_type,
       rip_redistribute_type_cmd,
       "redistribute " BANE_REDIST_STR_RIPD,
       REDIST_STR
       BANE_REDIST_HELP_STR_RIPD)
{
  int i;
  ...
  for(i = 0; redist_type[i].str; i++) 
	...
	{
	  zclient_redistribute (KROUTE_REDISTRIBUTE_ADD, zclient, 
	                        redist_type[i].type);
	  return CMD_SUCCESS;
	}
    }
...
DEFUN (no_rip_redistribute_type,
       ...
       "no redistribute " BANE_REDIST_STR_RIPD,
       NO_STR
       REDIST_STR
       BANE_REDIST_HELP_STR_RIPD)
{
...
DEFUN (rip_redistribute_type_routemap,
       ...
       "redistribute " BANE_REDIST_STR_RIPD " route-map WORD",
       REDIST_STR
       BANE_REDIST_HELP_STR_RIPD
       "Route map reference\n"
       "Pointer to route-map entries\n")
{
  ...
  for (i = 0; redist_type[i].str; i++) {
	...
	zclient_redistribute (KROUTE_REDISTRIBUTE_ADD, zclient, redist_type[i].type);
	return CMD_SUCCESS;
      }
  }
...
DEFUN (no_rip_redistribute_type_routemap,
       ...
       "no redistribute " BANE_REDIST_STR_RIPD " route-map WORD",
       NO_STR
       REDIST_STR
       BANE_REDIST_HELP_STR_RIPD
       "Route map reference\n"
...
DEFUN (rip_redistribute_type_metric,
       ...
       "redistribute " BANE_REDIST_STR_RIPD " metric <0-16>",
       REDIST_STR
       BANE_REDIST_HELP_STR_RIPD
       "Metric\n"
       "Metric value\n")
...
DEFUN (no_rip_redistribute_type_metric,
       ...
       "no redistribute " BANE_REDIST_STR_RIPD " metric <0-16>",
       NO_STR
       REDIST_STR
       BANE_REDIST_HELP_STR_RIPD
       "Metric\n"
...
DEFUN (rip_redistribute_type_metric_routemap,
       ...
       "redistribute " BANE_REDIST_STR_RIPD " metric <0-16> route-map WORD",
       REDIST_STR
       BANE_REDIST_HELP_STR_RIPD
       "Metric\n"
       "Metric value\n"
  ...
  if (! rip->default_information)
    {
      ...
      rip_redistribute_add (KROUTE_ROUTE_RIP, RIP_ROUTE_DEFAULT, &p, 0, 
                            NULL, 0, 0);
    }
  ...
  if (rip->default_information)
    {
      ...
      rip_redistribute_delete (KROUTE_ROUTE_RIP, RIP_ROUTE_DEFAULT, &p, 0);
    }
...
static int
config_write_kroute (struct vty *vty)
{
  if (! zclient->enable)
    {
  ...
  else if (! zclient->redist[KROUTE_ROUTE_RIP])
    {
	...
	if (config_mode)
	  {
		...
		if (rip->route_map[i].name)
		  vty_out (vty, " redistribute %s metric %d route-map %s%s",
			   kroute_route_string(i), rip->route_map[i].metric,
		...
		if (rip->route_map[i].name)
		  vty_out (vty, " redistribute %s route-map %s%s",
			   kroute_route_string(i), rip->route_map[i].name,
			   VTY_NEWLINE);
		else
...
rip_zclient_init ()
{
  /* Set default value to the kroute client structure. */
  zclient = zclient_new ();
  zclient_init (zclient, KROUTE_ROUTE_RIP);
  zclient->interface_add = rip_interface_add;
  zclient->interface_delete = rip_interface_delete;
  zclient->interface_address_add = rip_interface_address_add;
  zclient->interface_address_delete = rip_interface_address_delete;
  zclient->ipv4_route_add = rip_kroute_read_ipv4;
  zclient->ipv4_route_delete = rip_kroute_read_ipv4;
  zclient->interface_up = rip_interface_up;
  zclient->interface_down = rip_interface_down;
  
  /* Install kroute node. */
  install_node (&kroute_node, config_write_kroute);

  /* Install command elements to kroute node. */ 
  install_element (CONFIG_NODE, &router_kroute_cmd);
  install_element (CONFIG_NODE, &no_router_kroute_cmd);
  install_default (KROUTE_NODE);
  install_element (KROUTE_NODE, &rip_redistribute_rip_cmd);
  install_element (KROUTE_NODE, &no_rip_redistribute_rip_cmd);
