...
DEFUN (ospf_redistribute_source_metric_type,
       ...
       "redistribute " BANE_REDIST_STR_OSPFD 
         " metric <0-16777214> metric-type (1|2) route-map WORD",
       REDIST_STR
       BANE_REDIST_HELP_STR_OSPFD
       ...
       "Pointer to route-map entries\n")
{
  ...
  if (source < 0 || source == KROUTE_ROUTE_OSPF)
    return CMD_WARNING;
...
ALIAS (ospf_redistribute_source_metric_type,
       ...
       "redistribute " BANE_REDIST_STR_OSPFD " metric <0-16777214>",
       REDIST_STR
       BANE_REDIST_HELP_STR_OSPFD
       "Metric for redistributed routes\n"
       "OSPF default metric\n")
...
ALIAS (ospf_redistribute_source_type_metric,
       ...
       "redistribute " BANE_REDIST_STR_OSPFD " metric-type (1|2)",
       REDIST_STR
       BANE_REDIST_HELP_STR_OSPFD
       "OSPF exterior metric type for redistributed routes\n"
       "Set OSPF External Type 1 metrics\n"
...
DEFUN (ospf_redistribute_source_routemap,
       ...
       "redistribute " BANE_REDIST_STR_OSPFD " route-map WORD",
       REDIST_STR
       BANE_REDIST_HELP_STR_OSPFD
       "Route map reference\n"
       "Pointer to route-map entries\n")
...
DEFUN (no_ospf_redistribute_source,
       ...
       "no redistribute " BANE_REDIST_STR_OSPFD,
       NO_STR
       REDIST_STR
       BANE_REDIST_HELP_STR_OSPFD)
{
...
DEFUN (ospf_distribute_list_out,
       ...
       "distribute-list WORD out " BANE_REDIST_STR_OSPFD,
...
DEFUN (no_ospf_distribute_list_out,
       ...
       "no distribute-list WORD out " BANE_REDIST_STR_OSPFD,
       NO_STR
       "Filter networks in routing updates\n"
       "Access-list name\n"
       OUT_STR
       ...
       "Pointer to route-map entries\n")
{
  ...
  return ospf_redistribute_default_set (ospf, DEFAULT_ORIGINATE_KROUTE,
					type, metric);
}
       ...
       "Pointer to route-map entries\n")
{
  ...
  return ospf_redistribute_default_set (ospf, DEFAULT_ORIGINATE_KROUTE, -1, -1);
}
...
config_write_ospf_redistribute (struct vty *vty, struct ospf *ospf)
{
  ...
  for (type = 0; type < KROUTE_ROUTE_MAX; type++)
    if (type != zclient->redist_default && zclient->redist[type])
      {
        vty_out (vty, " redistribute %s", kroute_route_string(type));
  ...
  if (ospf)
	...
	if (DISTRIBUTE_NAME (ospf, type))
	  vty_out (vty, " distribute-list %s out %s%s", 
		   ...
		   kroute_route_string(type), VTY_NEWLINE);
...
ospf_vty_init (void)
{
  ...
  /* Init kroute related vty commands. */
  ospf_vty_kroute_init ();
}
