/*
    Monitor status of bane daemons and restart if necessary.
...
#ifdef PATH_WATCHBANE_PID
#define DEFAULT_PIDFILE		PATH_WATCHBANE_PID
#else
#define DEFAULT_PIDFILE		STATEDIR "/watchbane.pid"
#endif
...
/* Needs to be global, referenced somewhere inside libkroute. */
...
typedef enum
{
  ...
  PHASE_STOPS_PENDING,
  PHASE_WAITING_DOWN,
  PHASE_KROUTE_RESTART_PENDING,
  PHASE_WAITING_KROUTE_UP
} restart_phase_t;
...
static const char *phase_str[] =
{
  ...
  "Kroute restart job running",
  "Waiting for kroute to come up",
  "Start jobs running",
};
...
static struct global_state
{
  ...
  int loglevel;
  struct daemon *special;	/* points to kroute when doing phased restart */
  int numdaemons;
  int numpids;
...
Watchdog program to monitor status of bane daemons and try to restart\n\
...
0. Mode: %s.\n\
  Just monitor and report on status changes.  Example:\n\
    %s -d kroute ospfd bgpd\n\n\
1. Mode: %s.\n\
  Whenever any daemon hangs or crashes, use the given command to restart\n\
  them all.  Example:\n\
    %s -dz \\\n\
      -R '/sbin/service kroute restart; /sbin/service ospfd restart' \\\n\
      kroute ospfd\n\n\
2. Mode: %s.\n\
  ...
  in trouble using the supplied restart command.  Example:\n\
    %s -dz -r '/sbin/service %%s restart' kroute ospfd bgpd\n\n\
3. Mode: %s.\n\
  The same as the previous mode, except that there is special treatment when\n\
  the kroute daemon is in trouble.  In that case, a phased restart approach\n\
  is used: 1. stop all other daemons; 2. restart kroute; 3. start the other\n\
  daemons.  Example:\n\
...
-z, --unresponsive-restart\n\
		...
		restart purposes.\n\
-a, --all-restart\n\
		When kroute hangs or crashes, restart all daemons using\n\
		this phased approach: 1. stop all other daemons; 2. restart\n\
		kroute; 3. start other daemons.  Requires -r, -s, and -k.\n\
-A, --always-all-restart\n\
		When any daemon (not just kroute) hangs or crashes, use the\n\
...
phase_check(void)
{
  switch (gs.phase)
    ...
    case PHASE_WAITING_DOWN:
      ...
      set_phase(PHASE_KROUTE_RESTART_PENDING);
      /*FALLTHRU*/
    case PHASE_KROUTE_RESTART_PENDING:
      if (gs.special->restart.pid)
	break;
      zlog_info("Phased restart: %s restart job completed.",gs.special->name);
      set_phase(PHASE_WAITING_KROUTE_UP);
      /*FALLTHRU*/
    case PHASE_WAITING_KROUTE_UP:
      if (!IS_UP(gs.special))
        break;
...
try_restart(struct daemon *dmn)
{
  ...
  case MODE_SEPARATE_RESTART:
    ...
    break;
  case MODE_PHASED_KROUTE_RESTART:
    if (dmn != gs.special)
      {
...
main(int argc, char **argv)
{
  ...
  const char *special = "kroute";
  const char *blankstr = NULL;
  static struct bane_signal_t my_signals[] =
  {
    {
      .signal = SIGINT,
      .handler = sigint,
  ...
  while ((opt = getopt_long(argc, argv, "aAb:dek:l:m:M:i:p:r:R:S:s:t:T:zvh",
	...
	case 0:
	  ...
	  gs.mode = MODE_PHASED_KROUTE_RESTART;
	  break;
        case 'A':
	  if ((gs.mode != MODE_MONITOR) && (gs.mode != MODE_SEPARATE_RESTART))
	    {
...

  {
	...
	if (((gs.mode == MODE_PHASED_KROUTE_RESTART) ||
	     (gs.mode == MODE_PHASED_ALL_RESTART)) &&
  ...
  if (daemon_mode)
    {
      ...
      if (daemon (0, 0) < 0)
	{
	  fprintf(stderr, "Watchbane daemon failed: %s", strerror(errno));
	  exit (1);
	}
    }
  else
  ...
  /* Announce which daemons are being monitored. */
  {
      ...
      for (dmn = gs.daemons; dmn; dmn = dmn->next)
	{
      ...
      zlog_notice("%s %s watching [%s], mode [%s]",
      		  progname, BANE_VERSION, buf, mode_str[gs.mode]);
    }
  }
