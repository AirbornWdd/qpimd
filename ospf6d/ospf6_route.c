...
int g_ospf6_ph = 1; /*add by mosp, put header flag*/
...
ospf6_route_show (struct vty *vty, struct ospf6_route *route)
{
  ...
  /* this is mosp alter start...*/
  if (1 == g_ospf6_ph)
  {
      vty_out(vty,"flag(*: route is best)%s", VNL);
      vty_out(vty,"    dest type(?: Unknown, R: Router, N: Network, D: Discard, L: Linkstate, A:AddressRange)%s", VNL);
      vty_out(vty,"              path type(??: Unknown, IA: Intra-Area, IE: Inter-Area, E1: External-1, E2: External-2)%s", VNL);
      vty_out(vty,"%-23s %-30s %-25s %6.*s %s%s"," ", "destination", "nexthop", IFNAMSIZ, "ifname", "duration", VNL);
          
      /*clear put header once */
      g_ospf6_ph = 0;
  }
  /* this is mosp alter end.*/

//  vty_out (vty, "%c%1s %2s %-30s %-25s %6.*s %s%s", /* quagga org format*/
  vty_out (vty, "%c%-12s %-9s %-30s %-25s %6.*s %s%s",
                        ...
                        struct ospf6_route_table *table)
{
  ...
  g_ospf6_ph = 1; /* init g_ospf6_ph , add by mosp*/
