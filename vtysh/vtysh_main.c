...
usage (int status)
{
  if (status != 0)
    ...
    printf ("Usage : %s [OPTION...]\n\n" \
	    "Integrated shell for Bane routing software suite. \n\n" \
...
struct option longopts[] = 
{
  ...
  /* For compatibility with older kroute/bane versions */
...
main (int argc, char **argv, char **env)
{
  ...
  snprintf(history_file, sizeof(history_file), "%s/.history_bane", getenv("HOME"));
