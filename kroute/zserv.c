...
#include "kroute/ipforward.h"

/* Event list of kroute. */
enum event { KROUTE_SERV, KROUTE_READ, KROUTE_WRITE };

extern struct kroute_t krouted;

static void kroute_event (enum event event, int sock, struct zserv *client);

extern struct kroute_privs_t zserv_privs;

static void kroute_client_close (struct zserv *client);
...
zserv_delayed_close(struct thread *thread)
{
  ...
  kroute_client_close(client);
  return 0;
}

/* When client connects, it sends hello message
 * with promise to send kroute routes of specific type.
 * Kroute stores a socket fd of the client into
 * this array. And use it to clean up routes that
 * client didn't remove for some reasons after closing
 * connection.
 */
static int route_type_oaths[KROUTE_ROUTE_MAX];
...
zserv_flush_data(struct thread *thread)
{
  ...
  switch (buffer_flush_available(client->wb, client->sock))
    ...
    case BUFFER_ERROR:
      ...
      break;
    case BUFFER_PENDING:
      client->t_write = thread_add_write(krouted.master, zserv_flush_data,
      					 client, client->sock);
      break;
    case BUFFER_EMPTY:
      break;
...
static int
kroute_server_send_message(struct zserv *client)
{
  if (client->t_suicide)
    return -1;
  switch (buffer_write(client->wb, client->sock, STREAM_DATA(client->obuf),
    ...
    case BUFFER_ERROR:
	 ...
	 deleted. */
      client->t_suicide = thread_add_event(krouted.master, zserv_delayed_close,
					   client, 0);
      return -1;
    case BUFFER_EMPTY:
      ...
      break;
    case BUFFER_PENDING:
      THREAD_WRITE_ON(krouted.master, client->t_write,
...
zserv_create_header (struct stream *s, uint16_t cmd)
{
  ...
  stream_putw (s, KROUTE_HEADER_SIZE);
  stream_putc (s, KROUTE_HEADER_MARKER);
...
/* Interface is added. Send KROUTE_INTERFACE_ADD to client. */
/*
 * This function is called in the following situations:
 * - in response to a 3-byte KROUTE_INTERFACE_ADD request
 *   from the client.
 * - at startup, when kroute figures out the available interfaces
...
zsend_interface_add (struct zserv *client, struct interface *ifp)
{
  ...
  zserv_create_header (s, KROUTE_INTERFACE_ADD);
  zserv_encode_interface (s, ifp);

  return kroute_server_send_message(client);
}

/* Interface deletion from kroute daemon. */
int
zsend_interface_delete (struct zserv *client, struct interface *ifp)
{
  ...
  zserv_create_header (s, KROUTE_INTERFACE_DELETE);
  zserv_encode_interface (s, ifp);

  return kroute_server_send_message (client);
}

/* Interface address is added/deleted. Send KROUTE_INTERFACE_ADDRESS_ADD or
 * KROUTE_INTERFACE_ADDRESS_DELETE to the client. 
 *
 * A KROUTE_INTERFACE_ADDRESS_ADD is sent in the following situations:
 * - in response to a 3-byte KROUTE_INTERFACE_ADD request
 *   from the client, after the KROUTE_INTERFACE_ADD has been
 *   sent from kroute to the client
 ...
 * 
 * The call tree that triggers KROUTE_INTERFACE_ADDRESS_DELETE: 
 *
 *                   zsend_interface_address(DELETE)
 *                           ^                         
 *                           |                        
 *          kroute_interface_address_delete_update    
 *             ^                        ^      ^
 *             |                        |      if_delete_update
 *             |                        |
                         ...
                         struct interface *ifp, struct connected *ifc)
{
  ...
  /* 
   * XXX gnu version does not send prefixlen for KROUTE_INTERFACE_ADDRESS_DELETE
   * but kroute_interface_address_delete_read() in the gnu version 
   * expects to find it
   */
...
/*
 * The cmd passed to zsend_interface_update  may be KROUTE_INTERFACE_UP or
 * KROUTE_INTERFACE_DOWN.
 *
 * The KROUTE_INTERFACE_UP message is sent from the kroute server to
 ...
 * The KROUTE_INTERFACE_DOWN message is sent when an if_down is detected.
 */
int
...
/*
 * The kroute server sends the clients  a KROUTE_IPV4_ROUTE_ADD or a
 * KROUTE_IPV6_ROUTE_ADD via zsend_route_multipath in the following
 * situations:
 * - when the client starts up, and requests default information
 *   by sending a KROUTE_REDISTRIBUTE_DEFAULT_ADD to the kroute server, in the
 * - case of rip, ripngd, ospfd and ospf6d, when the client sends a
 *   KROUTE_REDISTRIBUTE_ADD as a result of the "redistribute" vty cmd,
 * - when the kroute server redistributes routes after it updates its rib
 *
 * The kroute server sends clients a KROUTE_IPV4_ROUTE_DELETE or a
 * KROUTE_IPV6_ROUTE_DELETE via zsend_route_multipath when:
 * - a "ip route"  or "ipv6 route" vty command is issued, a prefix is
 * - deleted from kroute's rib, and this info
 *   has to be redistributed to the clients 
 * 
 * XXX The KROUTE_IPV*_ROUTE_ADD message is also sent by the client to the
 * kroute server when the client wants to tell the kroute server to add a
                       ...
                       struct rib *rib)
{
  ...
  /* 
   * XXX The message format sent by kroute below does not match the format
   * of the corresponding message expected by the kroute server
   ...
   * nexthop KROUTE_NEXTHOP_IPV4 is never set, KROUTE_NEXTHOP_IFINDEX 
   * is hard-coded.
   */
  /* Nexthop */
...
#endif
              default:
                if (cmd == KROUTE_IPV4_ROUTE_ADD 
                    || cmd == KROUTE_IPV4_ROUTE_DELETE)
                  {
  ...
  /* Metric */
  if (cmd == KROUTE_IPV4_ROUTE_ADD || cmd == KROUTE_IPV6_ROUTE_ADD)
    {
...
zsend_ipv6_nexthop_lookup (struct zserv *client, struct in6_addr *addr)
{
  ...
  /* Fill in result. */
  zserv_create_header (s, KROUTE_IPV6_NEXTHOP_LOOKUP);
	...
	if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB))
	  {
	    ...
	    switch (nexthop->type)
	      {
	      case KROUTE_NEXTHOP_IPV6:
		stream_put (s, &nexthop->gate.ipv6, 16);
		break;
	      case KROUTE_NEXTHOP_IPV6_IFINDEX:
	      case KROUTE_NEXTHOP_IPV6_IFNAME:
		stream_put (s, &nexthop->gate.ipv6, 16);
		stream_putl (s, nexthop->ifindex);
		break;
	      case KROUTE_NEXTHOP_IFINDEX:
	      case KROUTE_NEXTHOP_IFNAME:
		stream_putl (s, nexthop->ifindex);
		break;
	      default:
                /* do nothing */
...
zsend_ipv4_nexthop_lookup (struct zserv *client, struct in_addr addr)
{
  ...
  /* Fill in result. */
  zserv_create_header (s, KROUTE_IPV4_NEXTHOP_LOOKUP);
...
zsend_ipv4_import_lookup (struct zserv *client, struct prefix_ipv4 *p)
{
  ...
  /* Fill in result. */
  zserv_create_header (s, KROUTE_IPV4_IMPORT_LOOKUP);
...

/* Router-id is updated. Send KROUTE_ROUTER_ID_ADD to client. */
int
zsend_router_id_update (struct zserv *client, struct prefix *p)
{
  ...
  /* Message type. */
  zserv_create_header (s, KROUTE_ROUTER_ID_UPDATE);
...

/* Register kroute server interface information.  Send current all
...
zread_interface_add (struct zserv *client, u_short length)
{
  ...
  for (ALL_LIST_ELEMENTS (iflist, ifnode, ifnnode, ifp))
	...
	{
	  if (CHECK_FLAG (c->conf, KROUTE_IFC_REAL) &&
	      (zsend_interface_address (KROUTE_INTERFACE_ADDRESS_ADD, client, 
				        ifp, c) < 0))
	    return -1;
	}
    }
...
/* Unregister kroute server interface information. */
static int
...
/* 
 * Parse the KROUTE_IPV4_ROUTE_ADD sent from client. Update rib and
 * add kernel route. 
 */
static int
zread_ipv4_add (struct zserv *client, u_short length)
{
  ...
  /* Table */
  rib->table=krouted.rtm_table_default;
  rib_add_ipv4_multipath (&p, rib, safi);
  return 0;
}

/* Kroute server IPv4 prefix delete function. */
static int
...
#ifdef HAVE_IPV6
/* Kroute server IPv6 prefix add function. */
static int
zread_ipv6_add (struct zserv *client, u_short length)
{
  ...
  if (IN6_IS_ADDR_UNSPECIFIED (&nexthop))
    rib_add_ipv6 (api.type, api.flags, &p, NULL, ifindex, krouted.rtm_table_default, api.metric,
		  api.distance, api.safi);
  else
    rib_add_ipv6 (api.type, api.flags, &p, &nexthop, ifindex, krouted.rtm_table_default, api.metric,
		  api.distance, api.safi);
  return 0;
}

/* Kroute server IPv6 prefix delete function. */
static int
...
/* Register kroute server router-id information.  Send current router-id */
static int
...
/* Unregister kroute server router-id information. */
static int
...
/* If client sent routes of specific type, kroute removes it
 * and returns number of deleted routes.
 */
static void
kroute_score_rib (int client_sock)
{
  int i;

  for (i = KROUTE_ROUTE_RIP; i < KROUTE_ROUTE_MAX; i++)
    if (client_sock == route_type_oaths[i])
      {
        zlog_notice ("client %d disconnected. %lu %s routes removed from the rib",
                      client_sock, rib_score_proto (i), kroute_route_string (i));
...
/* Close kroute client. */
static void
kroute_client_close (struct zserv *client)
{
  /* Close file descriptor. */
  if (client->sock)
    {
      close (client->sock);
      kroute_score_rib (client->sock);
      client->sock = -1;
    }
  ...
  listnode_delete (krouted.client_list, client);
  XFREE (0, client);
}

/* Make new client. */
static void
kroute_client_create (int sock)
{
  struct zserv *client;
  ...
  client->sock = sock;
  client->ibuf = stream_new (KROUTE_MAX_PACKET_SIZ);
  client->obuf = stream_new (KROUTE_MAX_PACKET_SIZ);
  client->wb = buffer_new(0);

  /* Set table number. */
  client->rtm_table = krouted.rtm_table_default;

  /* Add this client to linked list. */
  listnode_add (krouted.client_list, client);
  
  /* Make new read thread. */
  kroute_event (KROUTE_READ, sock, client);
}

/* Handler of kroute service request. */
static int
kroute_client_read (struct thread *thread)
{
  int sock;
  struct zserv *client;
  size_t already;
  ...
  if ((already = stream_get_endp(client->ibuf)) < KROUTE_HEADER_SIZE)
    {
      ssize_t nbyte;
      if (((nbyte = stream_read_try (client->ibuf, sock,
				     KROUTE_HEADER_SIZE-already)) == 0) ||
	  (nbyte == -1))
	{
      ...
      if (nbyte != (ssize_t)(KROUTE_HEADER_SIZE-already))
	{
	  /* Try again later. */
	  kroute_event (KROUTE_READ, sock, client);
	  return 0;
	}
      already = KROUTE_HEADER_SIZE;
    }
  ...
  if (marker != KROUTE_HEADER_MARKER || version != ZSERV_VERSION)
    {
  ...
  if (length < KROUTE_HEADER_SIZE) 
    {
      zlog_warn("%s: socket %d message length %u is less than header size %d",
	        __func__, sock, length, KROUTE_HEADER_SIZE);
      kroute_client_close (client);
      return -1;
    }
  ...
  if (already < length)
	...
	{
	  if (IS_KROUTE_DEBUG_EVENT)
	    zlog_debug ("connection closed [%d] when reading kroute data", sock);
	  kroute_client_close (client);
	  return -1;
	}
  ...
  length -= KROUTE_HEADER_SIZE;

  /* Debug packet information. */
  if (IS_KROUTE_DEBUG_EVENT)
    zlog_debug ("kroute message comes from socket [%d]", sock);

  if (IS_KROUTE_DEBUG_PACKET && IS_KROUTE_DEBUG_RECV)
    zlog_debug ("kroute message received [%s] %d", 
	       zserv_command_string (command), length);

  switch (command) 
    {
    case KROUTE_ROUTER_ID_ADD:
      zread_router_id_add (client, length);
      break;
    case KROUTE_ROUTER_ID_DELETE:
      zread_router_id_delete (client, length);
      break;
    case KROUTE_INTERFACE_ADD:
      zread_interface_add (client, length);
      break;
    case KROUTE_INTERFACE_DELETE:
      zread_interface_delete (client, length);
      break;
    case KROUTE_IPV4_ROUTE_ADD:
      zread_ipv4_add (client, length);
      break;
    case KROUTE_IPV4_ROUTE_DELETE:
      zread_ipv4_delete (client, length);
      break;
#ifdef HAVE_IPV6
    case KROUTE_IPV6_ROUTE_ADD:
      zread_ipv6_add (client, length);
      break;
    case KROUTE_IPV6_ROUTE_DELETE:
      zread_ipv6_delete (client, length);
      break;
#endif /* HAVE_IPV6 */
    case KROUTE_REDISTRIBUTE_ADD:
      kroute_redistribute_add (command, client, length);
      break;
    case KROUTE_REDISTRIBUTE_DELETE:
      kroute_redistribute_delete (command, client, length);
      break;
    case KROUTE_REDISTRIBUTE_DEFAULT_ADD:
      kroute_redistribute_default_add (command, client, length);
      break;
    case KROUTE_REDISTRIBUTE_DEFAULT_DELETE:
      kroute_redistribute_default_delete (command, client, length);
      break;
    case KROUTE_IPV4_NEXTHOP_LOOKUP:
      zread_ipv4_nexthop_lookup (client, length);
      break;
#ifdef HAVE_IPV6
    case KROUTE_IPV6_NEXTHOP_LOOKUP:
      zread_ipv6_nexthop_lookup (client, length);
      break;
#endif /* HAVE_IPV6 */
    case KROUTE_IPV4_IMPORT_LOOKUP:
    ...
    case KROUTE_HELLO:
      ...
      break;
    default:
      zlog_info ("Kroute received unknown command %d", command);
      break;
    }
...
/* Accept code of kroute server socket. */
static int
kroute_accept (struct thread *thread)
{
  int accept_sock;
  int client_sock;
  ...
  /* Reregister myself. */
  kroute_event (KROUTE_SERV, accept_sock, NULL);
  ...
  if (client_sock < 0)
    {
      zlog_warn ("Can't accept kroute socket: %s", safe_strerror (errno));
      return -1;
    }
  ...
  /* Create new kroute client. */
  kroute_client_create (client_sock);

  return 0;
}

#ifdef HAVE_TCP_KROUTE
/* Make kroute's server socket. */
static void
kroute_serv ()
{
  int ret;
  ...
  if (accept_sock < 0) 
    {
      zlog_warn ("Can't create zserv stream socket: %s", 
                 safe_strerror (errno));
      zlog_warn ("kroute can't provice full functionality due to above error");
      return;
    }
  ...
  addr.sin_port = htons (KROUTE_PORT);
...
#endif /* HAVE_TCP_KROUTE */

/* For sockaddr_un. */
#include <sys/un.h>

/* kroute server UNIX domain socket. */
static void
kroute_serv_un (const char *path)
{
  int ret;
  int sock, len;
  ...
  if (sock < 0)
    {
      zlog_warn ("Can't create zserv unix socket: %s", 
                 safe_strerror (errno));
      zlog_warn ("kroute can't provide full functionality due to above error");
      return;
    }
  ...
  kroute_event (KROUTE_SERV, sock, NULL);
}


static void
kroute_event (enum event event, int sock, struct zserv *client)
{
  switch (event)
    {
    case KROUTE_SERV:
      thread_add_read (krouted.master, kroute_accept, client, sock);
      break;
    case KROUTE_READ:
      client->t_read = 
	thread_add_read (krouted.master, kroute_client_read, client, sock);
      break;
    case KROUTE_WRITE:
      /**/
      break;
...
{
  vty_out (vty, "table %d%s", krouted.rtm_table_default,
	   VTY_NEWLINE);
  return CMD_SUCCESS;
}
...
DEFUN (config_table, 
       ...
       "TABLE integer\n")
{
  krouted.rtm_table_default = strtol (argv[0], (char**)0, 10);
  return CMD_SUCCESS;
}
...
DEFUN (show_kroute_client,
       show_kroute_client_cmd,
       "show kroute client",
       SHOW_STR
       "Kroute information"
       "Client information")
{
  ...
  for (ALL_LIST_ELEMENTS_RO (krouted.client_list, node, client))
...

/* Initialisation of kroute and installation of commands. */
void
kroute_init (void)
{
  /* Client list init. */
  krouted.client_list = list_new ();
  ...
  install_element (ENABLE_NODE, &show_kroute_client_cmd);
  ...
  /* Route-map */
  kroute_route_map_init ();
}

/* Make kroute server socket, wiping any existing one (see bug #403). */
void
kroute_zserv_socket_init (char *path)
{
#ifdef HAVE_TCP_KROUTE
  kroute_serv ();
#else
  kroute_serv_un (path ? path : KROUTE_SERV_PATH);
#endif /* HAVE_TCP_KROUTE */
}
