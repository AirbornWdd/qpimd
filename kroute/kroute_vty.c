...
static int
kroute_static_ipv4 (struct vty *vty, int add_cmd, const char *dest_str,
		   ...
		   const char *flag_str, const char *distance_str)
{
  ...
  if (distance_str)
    ...
    distance = KROUTE_STATIC_DISTANCE_DEFAULT;
  ...
  if ((gate_str != NULL) && (strncasecmp (gate_str, "Null0", strlen (gate_str)) == 0))
    {
      ...
      if (add_cmd)
        static_add_ipv4 (&p, NULL, NULL, KROUTE_FLAG_BLACKHOLE, distance, 0);
      else
  ...
  if (flag_str) {
    switch(flag_str[0]) {
      case 'r':
      case 'R': /* XXX */
        SET_FLAG (flag, KROUTE_FLAG_REJECT);
        break;
      case 'b':
      case 'B': /* XXX */
        SET_FLAG (flag, KROUTE_FLAG_BLACKHOLE);
        break;
      default:
...
DEFUN (ip_route, 
       ...
       "Null interface\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], NULL, argv[1], NULL, NULL);
}
...
{
  return kroute_static_ipv4 (vty, 1, argv[0], NULL, argv[1], argv[2], NULL);
}
...
DEFUN (ip_route_flags2,
       ...
       "Silently discard pkts when matched\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], NULL, NULL, argv[1], NULL);
}
...
DEFUN (ip_route_mask,
       ...
       "Null interface\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], argv[1], argv[2], NULL, NULL);
}
...
DEFUN (ip_route_mask_flags,
       ...
       "Silently discard pkts when matched\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], argv[1], argv[2], argv[3], NULL);
}
...
DEFUN (ip_route_mask_flags2,
       ...
       "Silently discard pkts when matched\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], argv[1], NULL, argv[2], NULL);
}
...
{
  return kroute_static_ipv4 (vty, 1, argv[0], NULL, argv[1], NULL, argv[2]);
}
...
DEFUN (ip_route_flags_distance,
       ...
       "Distance value for this route\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], NULL, argv[1], argv[2], argv[3]);
}
...
DEFUN (ip_route_flags_distance2,
       ...
       "Distance value for this route\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], NULL, NULL, argv[1], argv[2]);
}
...
DEFUN (ip_route_mask_distance,
       ...
       "Distance value for this route\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], argv[1], argv[2], NULL, argv[3]);
}
...
DEFUN (ip_route_mask_flags_distance,
       ...
       "Silently discard pkts when matched\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], argv[1], argv[2], argv[3], argv[4]);
}
...
DEFUN (ip_route_mask_flags_distance2,
       ...
       "Silently discard pkts when matched\n")
{
  return kroute_static_ipv4 (vty, 1, argv[0], argv[1], NULL, argv[2], argv[3]);
}
...
DEFUN (no_ip_route, 
       ...
       "Null interface\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], NULL, argv[1], NULL, NULL);
}
...
DEFUN (no_ip_route_flags2,
       ...
       "Silently discard pkts when matched\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], NULL, NULL, NULL, NULL);
}
...
DEFUN (no_ip_route_mask,
       ...
       "Null interface\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], argv[1], argv[2], NULL, NULL);
}
...
DEFUN (no_ip_route_mask_flags2,
       ...
       "Silently discard pkts when matched\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], argv[1], NULL, NULL, NULL);
}
...
DEFUN (no_ip_route_distance,
       ...
       "Distance value for this route\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], NULL, argv[1], NULL, argv[2]);
}
...
DEFUN (no_ip_route_flags_distance,
       ...
       "Distance value for this route\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], NULL, argv[1], argv[2], argv[3]);
}
...
DEFUN (no_ip_route_flags_distance2,
       ...
       "Distance value for this route\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], NULL, NULL, argv[1], argv[2]);
}
...
DEFUN (no_ip_route_mask_distance,
       ...
       "Distance value for this route\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], argv[1], argv[2], NULL, argv[3]);
}
...
DEFUN (no_ip_route_mask_flags_distance,
       ...
       "Distance value for this route\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], argv[1], argv[2], argv[3], argv[4]);
}
...
DEFUN (no_ip_route_mask_flags_distance2,
       ...
       "Distance value for this route\n")
{
  return kroute_static_ipv4 (vty, 0, argv[0], argv[1], NULL, argv[2], argv[3]);
}

char *proto_rm[AFI_MAX][KROUTE_ROUTE_MAX+1];	/* "any" == KROUTE_ROUTE_MAX */
  ...
  for (rib = rn->info; rib; rib = rib->next)
    {
      vty_out (vty, "Routing entry for %s/%d%s", 
	       ...
	       VTY_NEWLINE);
      vty_out (vty, "  Known via \"%s\"", kroute_route_string (rib->type));
      vty_out (vty, ", distance %d, metric %d", rib->distance, rib->metric);
      if (CHECK_FLAG (rib->flags, KROUTE_FLAG_SELECTED))
	vty_out (vty, ", best");
      if (rib->refcnt)
	vty_out (vty, ", refcnt %ld", rib->refcnt);
      if (CHECK_FLAG (rib->flags, KROUTE_FLAG_BLACKHOLE))
       vty_out (vty, ", blackhole");
      if (CHECK_FLAG (rib->flags, KROUTE_FLAG_REJECT))
       vty_out (vty, ", reject");
      vty_out (vty, "%s", VTY_NEWLINE);

#define ONE_DAY_SECOND 60*60*24
#define ONE_WEEK_SECOND 60*60*24*7
      if (rib->type == KROUTE_ROUTE_RIP
	  || rib->type == KROUTE_ROUTE_OSPF
	  || rib->type == KROUTE_ROUTE_BABEL
	  || rib->type == KROUTE_ROUTE_ISIS
	  || rib->type == KROUTE_ROUTE_BGP)
	{
	  time_t uptime;
	  struct tm *tm;
...
DEFUN (show_ip_route_protocol,
       ...
       "show ip route " BANE_IP_REDIST_STR_KROUTE,
       SHOW_STR
       IP_STR
       "IP routing table\n"
       BANE_IP_REDIST_HELP_STR_KROUTE)
{
  int type;
  struct route_table *table;
  struct route_node *rn;
...
vty_show_ip_route_summary (struct vty *vty, struct route_table *table)
{
  ...
  struct rib *rib;
  struct nexthop *nexthop;
#define KROUTE_ROUTE_IBGP  KROUTE_ROUTE_MAX
#define KROUTE_ROUTE_TOTAL (KROUTE_ROUTE_IBGP + 1)
  u_int32_t rib_cnt[KROUTE_ROUTE_TOTAL + 1];
  u_int32_t fib_cnt[KROUTE_ROUTE_TOTAL + 1];
  u_int32_t i;
  ...
  for (rn = route_top (table); rn; rn = route_next (rn))
    for (rib = rn->info; rib; rib = rib->next)
      for (nexthop = rib->nexthop; nexthop; nexthop = nexthop->next)
        {
	  rib_cnt[KROUTE_ROUTE_TOTAL]++;
	  rib_cnt[rib->type]++;
	  if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB)) 
	    {
	      fib_cnt[KROUTE_ROUTE_TOTAL]++;
	      fib_cnt[rib->type]++;
	    }
	  if (rib->type == KROUTE_ROUTE_BGP && 
	      CHECK_FLAG (rib->flags, KROUTE_FLAG_IBGP)) 
	    {
	      rib_cnt[KROUTE_ROUTE_IBGP]++;
	      if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB)) 
		fib_cnt[KROUTE_ROUTE_IBGP]++;
	    }
	}
	  ...
	  if (i == KROUTE_ROUTE_BGP)
	    {
	      vty_out (vty, "%-20s %-20d %-20d %s", "ebgp", 
		       rib_cnt[KROUTE_ROUTE_BGP] - rib_cnt[KROUTE_ROUTE_IBGP],
		       fib_cnt[KROUTE_ROUTE_BGP] - fib_cnt[KROUTE_ROUTE_IBGP],
		       VTY_NEWLINE);
	      vty_out (vty, "%-20s %-20d %-20d %s", "ibgp", 
		       rib_cnt[KROUTE_ROUTE_IBGP], fib_cnt[KROUTE_ROUTE_IBGP],
		       VTY_NEWLINE);
	    }
	  else 
	    vty_out (vty, "%-20s %-20d %-20d %s", kroute_route_string(i), 
		     rib_cnt[i], fib_cnt[i], VTY_NEWLINE);
	}
    }

  vty_out (vty, "------%s", VTY_NEWLINE);
  vty_out (vty, "%-20s %-20d %-20d %s", "Totals", rib_cnt[KROUTE_ROUTE_TOTAL], 
	   fib_cnt[KROUTE_ROUTE_TOTAL], VTY_NEWLINE);  
}
    ...
    for (si = rn->info; si; si = si->next)
      {
        ...
        if (si->type != STATIC_IPV4_BLACKHOLE)
          {
            if (CHECK_FLAG(si->flags, KROUTE_FLAG_REJECT))
              vty_out (vty, " %s", "reject");

            if (CHECK_FLAG(si->flags, KROUTE_FLAG_BLACKHOLE))
              vty_out (vty, " %s", "blackhole");
          }

        if (si->distance != KROUTE_STATIC_DISTANCE_DEFAULT)
       ...
       "IP protocol filtering status\n")
{
    ...
    {
        if (proto_rm[AFI_IP][i])
          vty_out (vty, "%-10s  : %-10s%s", kroute_route_string(i),
					proto_rm[AFI_IP][i],
					VTY_NEWLINE);
        else
          vty_out (vty, "%-10s  : none%s", kroute_route_string(i), VTY_NEWLINE);
    }
    if (proto_rm[AFI_IP][i])
...
#define ONE_DAY_SECOND 60*60*24
#define ONE_WEEK_SECOND 60*60*24*7
      if (rib->type == KROUTE_ROUTE_RIPNG
	  || rib->type == KROUTE_ROUTE_OSPF6
	  || rib->type == KROUTE_ROUTE_BABEL
	  || rib->type == KROUTE_ROUTE_ISIS
	  || rib->type == KROUTE_ROUTE_BGP)
	{
...
DEFUN (show_ipv6_route_protocol,
       ...
       "show ipv6 route " BANE_IP6_REDIST_STR_KROUTE,
       SHOW_STR
       IP_STR
       "IP routing table\n"
	BANE_IP6_REDIST_HELP_STR_KROUTE)
{
  int type;
  struct route_table *table;
  struct route_node *rn;
...
static int
kroute_ip_config (struct vty *vty)
{
  int write = 0;
...
{  
  ...
  for (i=0;i<KROUTE_ROUTE_MAX;i++)
    {
      if (proto_rm[AFI_IP][i])
        vty_out (vty, "ip protocol %s route-map %s%s", kroute_route_string(i),
                 proto_rm[AFI_IP][i], VTY_NEWLINE);
    }
  if (proto_rm[AFI_IP][KROUTE_ROUTE_MAX])
      vty_out (vty, "ip protocol %s route-map %s%s", "any",
               proto_rm[AFI_IP][KROUTE_ROUTE_MAX], VTY_NEWLINE);
...
/* Route VTY.  */
void
kroute_vty_init (void)
{
  install_node (&ip_node, kroute_ip_config);
