 ...
 * Copyright (C) 1997, 98 Kunihiro Ishiguro <kunihiro@kroute.org>
 *
...
rip_interface_new (void)
{
  ...
  /* Set default split-horizon behavior.  If the interface is Frame
     ...
     off.  But currently Kroute does detect Frame Relay or SMDS
...
int
rip_interface_down (int command, struct zclient *zclient, kroute_size_t length)
{
  struct interface *ifp;
  struct stream *s;
...
int
rip_interface_up (int command, struct zclient *zclient, kroute_size_t length)
{
  struct interface *ifp;
...
int
rip_interface_add (int command, struct zclient *zclient, kroute_size_t length)
{
  struct interface *ifp;
	...
	if ((rinfo = rp->info) != NULL)
	  {
		...
		if ((rinfo->ifindex == ifp->ifindex) &&
		    /* (rinfo->type != KROUTE_ROUTE_STATIC) && */
		    (rinfo->type != KROUTE_ROUTE_SYSTEM))
		  rip_redistribute_delete (rinfo->type,rinfo->sub_type,
					   (struct prefix_ipv4 *)&rp->p,
					   rinfo->ifindex);
	      }
...
rip_apply_address_add (struct connected *ifc)
{
  ...
  if ((rip_enable_if_lookup(ifc->ifp->name) >= 0) ||
    ...
    rip_redistribute_add(KROUTE_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
			   ...
			   kroute_size_t length)
{
  ...
  ifc = kroute_interface_address_read (KROUTE_INTERFACE_ADDRESS_ADD, 
                                      zclient->ibuf);
...
rip_apply_address_del (struct connected *ifc) {
  ...
  rip_redistribute_delete(KROUTE_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
			      ...
			      kroute_size_t length)
{
  ...
  ifc = kroute_interface_address_read (KROUTE_INTERFACE_ADDRESS_DELETE,
                                      zclient->ibuf);
  ...
  for (ALL_LIST_ELEMENTS (ifp->connected, node, nnode, connected))
    {
      ...
      if (set) {
      ...
      } else
        {
          rip_redistribute_delete (KROUTE_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
                                   &address, connected->ifp->ifindex);
          if (rip_redistribute_check (KROUTE_ROUTE_CONNECT))
            rip_redistribute_add (KROUTE_ROUTE_CONNECT, RIP_ROUTE_REDISTRIBUTE,
                                  &address, connected->ifp->ifindex,
                                  NULL, 0, 0);
        }
    }
