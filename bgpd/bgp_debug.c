...
unsigned long conf_bgp_debug_kroute;
...
unsigned long term_bgp_debug_kroute;
...
DEFUN (debug_bgp_kroute,
       debug_bgp_kroute_cmd,
       "debug bgp kroute",
       DEBUG_STR
       BGP_STR
       "BGP Kroute messages\n")
{
  if (vty->node == CONFIG_NODE)
  ...
  else
    {
      ...
      vty_out (vty, "BGP kroute debugging is on%s", VTY_NEWLINE);
    }
  return CMD_SUCCESS;
}

DEFUN (no_debug_bgp_kroute,
       no_debug_bgp_kroute_cmd,
       "no debug bgp kroute",
       NO_STR
       DEBUG_STR
       BGP_STR
       "BGP Kroute messages\n")
{
  if (vty->node == CONFIG_NODE)
  ...
  else
    {
      ...
      vty_out (vty, "BGP kroute debugging is off%s", VTY_NEWLINE);
    }
  return CMD_SUCCESS;
}

ALIAS (no_debug_bgp_kroute,
       undebug_bgp_kroute_cmd,
       "undebug bgp kroute",
       UNDEBUG_STR
       BGP_STR
       "BGP Kroute messages\n")
...
bgp_config_write_debug (struct vty *vty)
{
  ...
  if (CONF_BGP_DEBUG (kroute, KROUTE))
    {
      vty_out (vty, "debug bgp kroute%s", VTY_NEWLINE);
      write++;
    }
...
bgp_debug_init (void)
{
  ...
  install_element (ENABLE_NODE, &debug_bgp_kroute_cmd);
  install_element (CONFIG_NODE, &debug_bgp_kroute_cmd);
  ...
  install_element (ENABLE_NODE, &no_debug_bgp_kroute_cmd);
  install_element (ENABLE_NODE, &undebug_bgp_kroute_cmd);
  install_element (CONFIG_NODE, &no_debug_bgp_kroute_cmd);
