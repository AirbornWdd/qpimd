...
unsigned long ripng_debug_kroute = 0;

       ...
       "RIPng configuration\n")
{
  ...
  if (IS_RIPNG_DEBUG_KROUTE)
    vty_out (vty, "  RIPng kroute debugging is on%s", VTY_NEWLINE);
...
DEFUN (debug_ripng_kroute,
       debug_ripng_kroute_cmd,
       "debug ripng kroute",
       DEBUG_STR
       "RIPng configuration\n"
       "Debug option set for ripng and kroute communication\n")
{
  ripng_debug_kroute = RIPNG_DEBUG_KROUTE;
  return CMD_WARNING;
}
...
DEFUN (no_debug_ripng_kroute,
       no_debug_ripng_kroute_cmd,
       "no debug ripng kroute",
       NO_STR
       DEBUG_STR
       "RIPng configuration\n"
       "Debug option set for ripng and kroute communication\n")
{
  ripng_debug_kroute = 0;
  return CMD_WARNING;
}
...
config_write_debug (struct vty *vty)
{
  ...
  if (IS_RIPNG_DEBUG_KROUTE)
    {
      vty_out (vty, "debug ripng kroute%s", VTY_NEWLINE);
      write++;
    }
  return write;
}
...
ripng_debug_init ()
{
  ...
  install_element (ENABLE_NODE, &debug_ripng_kroute_cmd);
  ...
  install_element (ENABLE_NODE, &no_debug_ripng_kroute_cmd);
  ...
  install_element (CONFIG_NODE, &debug_ripng_kroute_cmd);
  ...
  install_element (CONFIG_NODE, &no_debug_ripng_kroute_cmd);
}
