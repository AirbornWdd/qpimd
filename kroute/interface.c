...
static int
if_kroute_new_hook (struct interface *ifp)
{
  struct kroute_if *kroute_if;

  kroute_if = XCALLOC (MTYPE_TMP, sizeof (struct kroute_if));

  kroute_if->multicast = IF_KROUTE_MULTICAST_UNSPEC;
  kroute_if->shutdown = IF_KROUTE_SHUTDOWN_UNSPEC;
  ...
  kroute_if->ipv4_subnets = route_table_init ();
...
static int
if_kroute_delete_hook (struct interface *ifp)
{
  struct kroute_if *kroute_if;
  ...
  if (ifp->info)
    {
      ...
      if (kroute_if->ipv4_subnets)
	route_table_finish (kroute_if->ipv4_subnets);

      XFREE (MTYPE_TMP, kroute_if);
    }
...
if_subnet_add (struct interface *ifp, struct connected *ifc)
{
  ...
  cp = *ifc->address;
  apply_mask (&cp);
  rn = route_node_get (kroute_if->ipv4_subnets, &cp);
...
if_subnet_delete (struct interface *ifp, struct connected *ifc)
{
  ...
  rn = route_node_lookup (kroute_if->ipv4_subnets, ifc->address);
  if (! (rn && rn->info))
    return -1;
 ...
 * struct kroute_if.primary_state.
 *
...
{
  struct kroute_if *if_data;

  if_data = ifp->info;
  if (if_data->multicast == IF_KROUTE_MULTICAST_ON)
    if_set_flags (ifp, IFF_MULTICAST);
  else if (if_data->multicast == IF_KROUTE_MULTICAST_OFF)
...
if_delete_update (struct interface *ifp)
{
  ...
  if (ifp->connected)
      ...
      while ((node = (last ? last->next : listhead (ifp->connected))))
	{
	  ...
	  if (p->family == AF_INET
	      && (rn = route_node_lookup (kroute_if->ipv4_subnets, p)))
	    {
...
{
  struct kroute_if *zif;
  struct rtadvconf *rtadv;
  int interval;

  zif = (struct kroute_if *) ifp->info;
  rtadv = &zif->rtadv;
...
if_dump_vty (struct vty *vty, struct interface *ifp)
{
  ...
  for (rn = route_top (kroute_if->ipv4_subnets); rn; rn = route_next (rn))
    {
      if (! rn->info)
	continue;
       ...
       "Set multicast flag to interface\n")
{
  ...
  if_data = ifp->info;
  if_data->multicast = IF_KROUTE_MULTICAST_ON;
       ...
       "Unset multicast flag to interface\n")
{
  ...
  if_data = ifp->info;
  if_data->multicast = IF_KROUTE_MULTICAST_OFF;
...
if_config_write (struct vty *vty)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (iflist, node, ifp))
      ...
      if (if_data)
	{
	  ...
	  if (if_data->multicast != IF_KROUTE_MULTICAST_UNSPEC)
	    vty_out (vty, " %smulticast%s",
		     if_data->multicast == IF_KROUTE_MULTICAST_ON ? "" : "no ",
		     VTY_NEWLINE);
	}
...
kroute_if_init (void)
{
  ...
  if_init ();
  if_add_hook (IF_NEW_HOOK, if_kroute_new_hook);
  if_add_hook (IF_DELETE_HOOK, if_kroute_delete_hook);
