...
/* Is the interface operative, eg. either UP & RUNNING
   or UP & !KROUTE_INTERFACE_LINK_DETECTION */
int
if_is_operative (struct interface *ifp)
{
  return ((ifp->flags & IFF_UP) &&
	  (ifp->flags & IFF_RUNNING || !CHECK_FLAG(ifp->status, KROUTE_INTERFACE_LINKDETECTION)));
}
...

#ifdef SUNOS_5
/* Need to handle upgrade from SUNWkroute to Bane. SUNWkroute created
 ...
 * interfaces can be fruitless, for that reason Bane only tracks
 * the primary IP interface.
 *
 * We try accomodate SUNWkroute by:
 * - looking up the interface name, to see whether it exists, if so
 *   its useable
 *   - for protocol daemons, this could only because kroute told us of
 *     the interface
 *   - for kroute, only because it learnt from kernel
 * - if not:
...
if_sunwkroute_get (const char *name, size_t nlen)
{
  struct interface *ifp;
  size_t seppos = 0;
...
#ifdef SUNOS_5
  ifp = if_sunwkroute_get (argv[0], sl);
#else
