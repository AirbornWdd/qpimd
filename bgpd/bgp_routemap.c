...
bgp_route_map_update (const char *unused)
{
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
	...
	{
	  if (bgp->rmap[KROUTE_FAMILY_IPV4][i].name)
	    bgp->rmap[KROUTE_FAMILY_IPV4][i].map = 
	      route_map_lookup_by_name (bgp->rmap[KROUTE_FAMILY_IPV4][i].name);
#ifdef HAVE_IPV6
	  if (bgp->rmap[KROUTE_FAMILY_IPV6][i].name)
	    bgp->rmap[KROUTE_FAMILY_IPV6][i].map =
	      route_map_lookup_by_name (bgp->rmap[KROUTE_FAMILY_IPV6][i].name);
#endif /* HAVE_IPV6 */
	}
    }
}
