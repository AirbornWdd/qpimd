...
static struct bane_signal_t bgp_signals[] = 
{
  { 
    .signal = SIGHUP, 
    .handler = &sighup,
...
struct kroute_privs_t bgpd_privs =
{
...
/*
  ...
  Kroute route removal and protocol teardown are not meant to be done here.
...
bgp_exit (int status)
{
  ...
  /* reverse bgp_kroute_init/if_init */
  if (retain_mode)
...
main (int argc, char **argv)
{
  ...
  /* Print banner. */
  zlog_notice ("BGPd %s starting: vty@%d, bgp@%s:%d", BANE_VERSION,
	       vty_port, 
