...
int
ripng_interface_up (int command, struct zclient *zclient, kroute_size_t length)
{
  struct stream *s;
...
int
ripng_interface_add (int command, struct zclient *zclient, kroute_size_t length)
{
  struct interface *ifp;
...
ripng_apply_address_add (struct connected *ifc) {
  ...
  if ((ripng_enable_if_lookup(ifc->ifp->name) >= 0) ||
    ...
    ripng_redistribute_add(KROUTE_ROUTE_CONNECT, RIPNG_ROUTE_INTERFACE,
...
ripng_apply_address_del (struct connected *ifc) {
  ...
  ripng_redistribute_delete(KROUTE_ROUTE_CONNECT, RIPNG_ROUTE_INTERFACE,
  ...
  for (ALL_LIST_ELEMENTS (ifp->connected, node, nnode, connected))
    {
      ...
      if (set) {
        ...
        if (ripng_redistribute_check (KROUTE_ROUTE_CONNECT))
          ripng_redistribute_add (KROUTE_ROUTE_CONNECT, RIPNG_ROUTE_REDISTRIBUTE,
                                  &address, connected->ifp->ifindex, NULL);
      }
    }
}
