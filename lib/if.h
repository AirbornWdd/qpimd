...
#define IFINDEX_INTERNAL	0
  ...
  /* Kroute internal interface status */
  u_char status;
#define KROUTE_INTERFACE_ACTIVE     (1 << 0)
#define KROUTE_INTERFACE_SUB        (1 << 1)
#define KROUTE_INTERFACE_LINKDETECTION (1 << 2)
...
struct connected
{
  ...
  u_char conf;
#define KROUTE_IFC_REAL         (1 << 0)
#define KROUTE_IFC_CONFIGURED   (1 << 1)
  /*
     The KROUTE_IFC_REAL flag should be set if and only if this address
     exists in the kernel.
     The KROUTE_IFC_CONFIGURED flag should be set if and only if this address
     was configured by the user from inside bane.
   */

  /* Flags for connected address. */
  u_char flags;
#define KROUTE_IFA_SECONDARY    (1 << 0)
#define KROUTE_IFA_PEER         (1 << 1)
  /* N.B. the KROUTE_IFA_PEER flag should be set if and only if
  ...
  /* Peer or Broadcast address, depending on whether KROUTE_IFA_PEER is set.
     Note: destination may be NULL if KROUTE_IFA_PEER is not set. */
...
#define CONNECTED_PEER(C) CHECK_FLAG((C)->flags, KROUTE_IFA_PEER)
