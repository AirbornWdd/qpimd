...
#define ZROUTE_NAME(x) kroute_route_string(x)
...
{
  ospf6_kroute_redistribute (type);
}
...
ospf6_asbr_redistribute_unset (int type)
{
  ...
  ospf6_kroute_no_redistribute (type);
                             ...
                             u_int nexthop_num, struct in6_addr *nexthop)
{
  ...
  if (! ospf6_kroute_is_redistribute (type))
    return;
...
DEFUN (ospf6_redistribute,
       ...
       "redistribute " BANE_REDIST_STR_OSPF6D,
       "Redistribute\n"
       BANE_REDIST_HELP_STR_OSPF6D
      )
{
  int type;

  type = proto_redistnum(AFI_IP6, argv[0]);
  if (type < 0 || type == KROUTE_ROUTE_OSPF6)
    return CMD_WARNING;
...
DEFUN (ospf6_redistribute_routemap,
       ...
       "redistribute " BANE_REDIST_STR_OSPF6D " route-map WORD",
       "Redistribute\n"
       BANE_REDIST_HELP_STR_OSPF6D
       "Route map reference\n"
       "Route map name\n"
...
DEFUN (no_ospf6_redistribute,
       ...
       "no redistribute " BANE_REDIST_STR_OSPF6D,
       NO_STR
       "Redistribute\n"
       BANE_REDIST_HELP_STR_OSPF6D
      )
