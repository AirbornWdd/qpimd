/* Bane signal handling functions.
 ...
 */
static void
bane_signal_handler (int signo)
{
  int i;
  struct bane_signal_t *sig;
...
bane_sigevent_process (void)
{
  ...
  if ( (sigprocmask (SIG_BLOCK, &newmask, &oldmask)) < 0)
    {
      zlog_err ("bane_signal_timer: couldnt block signals!");
      return -1;
    }
...
int
bane_signal_timer (struct thread *t)
{
  struct bane_sigevent_master_t *sigm;
  struct bane_signal_t *sig;
  int i;

  sigm = THREAD_ARG (t);
  sigm->t = thread_add_timer (sigm->t->master, bane_signal_timer, &sigmaster,
                              BANE_SIGNAL_TIMER_INTERVAL);
  return bane_sigevent_process ();
}
...
signal_set (int signo)
{
  ...
  sig.sa_handler = &bane_signal_handler;
...
#ifdef SIGEVENT_SCHEDULE_THREAD  
  sigmaster.t = 
    thread_add_timer (m, bane_signal_timer, &sigmaster, 
                      BANE_SIGNAL_TIMER_INTERVAL);
#endif /* SIGEVENT_SCHEDULE_THREAD */
}
