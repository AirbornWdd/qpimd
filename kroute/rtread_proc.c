...
proc_route_read (void)
{
  ...
  while (fgets (buf, RT_BUFSIZ, fp) != NULL)
	...
	kroute_flags |= KROUTE_FLAG_SELFROUTE;
      ...
      rib_add_ipv4 (KROUTE_ROUTE_KERNEL, kroute_flags, &p, &gateway, NULL, 0, 0, 0, 0, SAFI_UNICAST);
    }
...
proc_ipv6_route_read ()
{
  ...
  while (fgets (buf, RT_BUFSIZ, fp) != NULL)
	...
	kroute_flags |= KROUTE_FLAG_SELFROUTE;
      ...
      rib_add_ipv6 (KROUTE_ROUTE_KERNEL, kroute_flags, &p, &gateway, 0, 0,
		    metric, 0);
    }
