...
## - redistribute strings for the various Bane daemons
##
...
sub collect {
	...
	my (@names, @help) = ((), ());
	for my $p (@protos) {
		next if ($protodetail{$p}->{"daemon"} eq $daemon && $daemon ne "kroute");
...
for my $daemon (sort keys %daemons) {
	...
	if ($daemons{$daemon}->{"ipv4"} && $daemons{$daemon}->{"ipv6"}) {
		...
		printf "#define BANE_REDIST_STR_%s \\\n  %s\n", uc $daemon, $names;
		printf "#define BANE_REDIST_HELP_STR_%s \\\n%s\n", uc $daemon, $help;
		($names, $help) = collect($daemon, 1, 0);
		printf "#define BANE_IP_REDIST_STR_%s \\\n  %s\n", uc $daemon, $names;
		printf "#define BANE_IP_REDIST_HELP_STR_%s \\\n%s\n", uc $daemon, $help;
		($names, $help) = collect($daemon, 0, 1);
		printf "#define BANE_IP6_REDIST_STR_%s \\\n  %s\n", uc $daemon, $names;
		printf "#define BANE_IP6_REDIST_HELP_STR_%s \\\n%s\n", uc $daemon, $help;
	} else {
