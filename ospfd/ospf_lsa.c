...
/* from "edward rrr" <edward_rrr@hotmail.com>
   http://marc.theaimsgroup.com/?l=kroute&m=100739222210507&w=2 */
static int
...
metric_value (struct ospf *ospf, u_char src)
{
  if (ospf->dmetric[src].value < 0)
	...
	{
	  if (ospf->default_originate == DEFAULT_ORIGINATE_KROUTE)
...
ospf_default_external_info (struct ospf *ospf)
{
  ...
  for (type = 0; type <= KROUTE_ROUTE_MAX; type++)
    if (EXTERNAL_INFO (type) && type != KROUTE_ROUTE_OSPF)
      {
...
#if 0
  /* Remove External route from Kroute. */
  ospf_kroute_delete ((struct prefix_ipv4 *) p, &nexthop);
#endif
...
ospf_refresher_register_lsa (struct ospf *ospf, struct ospf_lsa *lsa)
{
  ...
  if (lsa->refresh_list < 0)
	...
	delay = 0;
      ...
      current_index = ospf->lsa_refresh_queue.index + (bane_time (NULL)
...
ospf_lsa_refresh_walker (struct thread *t)
{
  ...
  ospf->lsa_refresher_started = bane_time (NULL);
