...
vty_time_print (struct vty *vty, int cr)
{
  ...
  if (bane_timestamp(0, buf, sizeof(buf)) == 0)
    {
      zlog (NULL, LOG_INFO, "bane_timestamp error");
      return;
    }
  if (cr)
    vty_out (vty, "%s\n", buf);
...
vty_create (int vty_sock, union sockunion *su)
{
  ...
  {
      /* vty->node = AUTH_NODE; */
      /* 为了使用户登录时直接进入特权模式，故把AUTH_NODE改成ENABLE_NODE, alter by mosp, 2012/07/06 15:40:37   */
      vty->node = ENABLE_NODE;      
  }
...
#if 0  /* 为了登录时不输入密码更加方便，shade by mosp, 2012/07/06 15:14:00 */
  if (! no_password_check)
    {
                 ...
                 char *config_default_dir)
{
  ...
  if (config_file != NULL)
  ...
  else
    {
      ...
      /* Stat for vtysh Kroute.conf, if found startup and wait for
       * boot configuration
       */
