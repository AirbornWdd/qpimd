...
extern struct kroute_privs_t ripngd_privs;
...
{
  return (rinfo->type == KROUTE_ROUTE_RIPNG && rinfo->sub_type == RIPNG_ROUTE_RTE);
}
...
ripng_timeout (struct thread *t)
{
  ...
  ripng_kroute_ipv6_delete ((struct prefix_ipv6 *)&rp->p, &rinfo->nexthop,
				rinfo->ifindex);
		     ...
		     struct interface *ifp)
{
  ...
  if (ri->routemap[RIPNG_FILTER_IN])
    {
      ...
      newinfo.type = KROUTE_ROUTE_RIPNG;
  ...
  /* Kroute ripngd can handle offset-list in. */
  ...
  if (rinfo)
    {
      ...
      if (rinfo->type != KROUTE_ROUTE_RIPNG
	  && rinfo->metric != RIPNG_METRIC_INFINITY)
	return;
  ...
  if (rp->info == NULL)
      ...
      if (rte->metric != RIPNG_METRIC_INFINITY)
	{
	  ...
	  rinfo->type = KROUTE_ROUTE_RIPNG;
	  rinfo->sub_type = RIPNG_ROUTE_RTE;

	  ripng_kroute_ipv6_add (&p, &rinfo->nexthop, rinfo->ifindex,
				rinfo->metric);
	  rinfo->flags |= RIPNG_RTF_FIB;
	  ...
	  rte->metric < rinfo->metric)
	{
	  ...
	  if (oldmetric == RIPNG_METRIC_INFINITY &&
		...
		IPV6_ADDR_COPY (&rinfo->nexthop, nexthop);
	      ...
	      ripng_kroute_ipv6_add (&p, nexthop, ifp->ifindex, rinfo->metric);
	      rinfo->flags |= RIPNG_RTF_FIB;
	  ...
	  if (oldmetric != RIPNG_METRIC_INFINITY)
	    {
	      ripng_kroute_ipv6_delete (&p, &rinfo->nexthop, rinfo->ifindex);
	      ripng_kroute_ipv6_add (&p, nexthop, ifp->ifindex, rinfo->metric);
	      rinfo->flags |= RIPNG_RTF_FIB;
  ...
  if (rinfo)
    {
      ...
      if (rinfo->type == KROUTE_ROUTE_RIPNG
          && ((rinfo->sub_type == RIPNG_ROUTE_STATIC) ||
              (rinfo->sub_type == RIPNG_ROUTE_DEFAULT)) ) {
        if (type != KROUTE_ROUTE_RIPNG || ((sub_type != RIPNG_ROUTE_STATIC) &&
       ...
       "Set static RIPng route announcement\n")
{
  ...
  ripng_redistribute_add (KROUTE_ROUTE_RIPNG, RIPNG_ROUTE_STATIC, &p, 0, NULL);
       ...
       "Delete static RIPng route announcement\n")
{
  ...
  ripng_redistribute_delete (KROUTE_ROUTE_RIPNG, RIPNG_ROUTE_STATIC, &p, 0);
  route_unlock_node (rp);
       ...
       "Distribute default route\n")
{
  ...
  if (! ripng ->default_information) {
    ...
    str2prefix_ipv6 ("::/0", &p);
    ripng_redistribute_add (KROUTE_ROUTE_RIPNG, RIPNG_ROUTE_DEFAULT, &p, 0, NULL);
  }
       ...
       "Distribute default route\n")
{
  ...
  if (ripng->default_information) {
    ...
    str2prefix_ipv6 ("::/0", &p);
    ripng_redistribute_delete (KROUTE_ROUTE_RIPNG, RIPNG_ROUTE_DEFAULT, &p, 0);
  }
  ...
  if (ripng) {
    ...
    for (rp = route_top (ripng->table); rp; rp = route_next (rp)) {
      if ((rinfo = rp->info) != NULL) {
        if ((rinfo->type == KROUTE_ROUTE_RIPNG) &&
            (rinfo->sub_type == RIPNG_ROUTE_RTE))
          ripng_kroute_ipv6_delete ((struct prefix_ipv6 *)&rp->p,
