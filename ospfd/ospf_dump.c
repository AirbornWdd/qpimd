...
unsigned long conf_debug_ospf_kroute = 0;
...
unsigned long term_debug_ospf_kroute = 0;
...
{
  return (route_type == KROUTE_ROUTE_MAX) ?
  	 "Default" : kroute_route_string(route_type);
}
...

DEFUN (debug_ospf_kroute,
       debug_ospf_kroute_cmd,
       "debug ospf kroute",
       DEBUG_STR
       OSPF_STR
       "OSPF Kroute information\n")
{
  if (vty->node == CONFIG_NODE)
    {
      if (argc == 0)
	DEBUG_ON (kroute, KROUTE);
      else if (argc == 1)
	{
	  if (strncmp (argv[0], "i", 1) == 0)
	    DEBUG_ON (kroute, KROUTE_INTERFACE);
	  else if (strncmp (argv[0], "r", 1) == 0)
	    DEBUG_ON (kroute, KROUTE_REDISTRIBUTE);
	}
  ...
  /* ENABLE_NODE. */
  if (argc == 0)
    TERM_DEBUG_ON (kroute, KROUTE);
  else if (argc == 1)
    {
      if (strncmp (argv[0], "i", 1) == 0)
	TERM_DEBUG_ON (kroute, KROUTE_INTERFACE);
      else if (strncmp (argv[0], "r", 1) == 0)
	TERM_DEBUG_ON (kroute, KROUTE_REDISTRIBUTE);
    }

  return CMD_SUCCESS;
}

ALIAS (debug_ospf_kroute,
       debug_ospf_kroute_sub_cmd,
       "debug ospf kroute (interface|redistribute)",
       DEBUG_STR
       OSPF_STR
       "OSPF Kroute information\n"
       "Kroute interface\n"
...
DEFUN (no_debug_ospf_kroute,
       no_debug_ospf_kroute_cmd,
       "no debug ospf kroute",
       NO_STR
       DEBUG_STR
       OSPF_STR
       "OSPF Kroute information\n")
{
  if (vty->node == CONFIG_NODE)
    {
      if (argc == 0)
	DEBUG_OFF (kroute, KROUTE);
      else if (argc == 1)
	{
	  if (strncmp (argv[0], "i", 1) == 0)
	    DEBUG_OFF (kroute, KROUTE_INTERFACE);
	  else if (strncmp (argv[0], "r", 1) == 0)
	    DEBUG_OFF (kroute, KROUTE_REDISTRIBUTE);
	}
  ...
  /* ENABLE_NODE. */
  if (argc == 0)
    TERM_DEBUG_OFF (kroute, KROUTE);
  else if (argc == 1)
    {
      if (strncmp (argv[0], "i", 1) == 0)
	TERM_DEBUG_OFF (kroute, KROUTE_INTERFACE);
      else if (strncmp (argv[0], "r", 1) == 0)
	TERM_DEBUG_OFF (kroute, KROUTE_REDISTRIBUTE);
    }

  return CMD_SUCCESS;
}

ALIAS (no_debug_ospf_kroute,
       no_debug_ospf_kroute_sub_cmd,
       "no debug ospf kroute (interface|redistribute)",
       NO_STR
       DEBUG_STR
       OSPF_STR
       "OSPF Kroute information\n"
       ...
       OSPF_STR)
{
  ...
  /* Show debug status for Kroute. */
  if (IS_DEBUG_OSPF (kroute, KROUTE) == OSPF_DEBUG_KROUTE)
    vty_out (vty, "  OSPF Kroute debugging is on%s", VTY_NEWLINE);
  else
    {
      if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
	vty_out (vty, "  OSPF Kroute interface debugging is on%s", VTY_NEWLINE);
      if (IS_DEBUG_OSPF (kroute, KROUTE_REDISTRIBUTE))
	vty_out (vty, "  OSPF Kroute redistribute debugging is on%s", VTY_NEWLINE);
    }
...
config_write_debug (struct vty *vty)
{
  ...
  /* debug ospf kroute (interface|redistribute). */
  if (IS_CONF_DEBUG_OSPF (kroute, KROUTE) == OSPF_DEBUG_KROUTE)
    vty_out (vty, "debug ospf kroute%s", VTY_NEWLINE);
  else
    {
      if (IS_CONF_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
	vty_out (vty, "debug ospf kroute interface%s", VTY_NEWLINE);
      if (IS_CONF_DEBUG_OSPF (kroute, KROUTE_REDISTRIBUTE))
	vty_out (vty, "debug ospf kroute redistribute%s", VTY_NEWLINE);
...
debug_init ()
{
  ...
  install_element (ENABLE_NODE, &debug_ospf_kroute_sub_cmd);
  install_element (ENABLE_NODE, &debug_ospf_kroute_cmd);
  ...
  install_element (ENABLE_NODE, &no_debug_ospf_kroute_sub_cmd);
  install_element (ENABLE_NODE, &no_debug_ospf_kroute_cmd);
  ...
  install_element (CONFIG_NODE, &debug_ospf_kroute_sub_cmd);
  install_element (CONFIG_NODE, &debug_ospf_kroute_cmd);
  ...
  install_element (CONFIG_NODE, &no_debug_ospf_kroute_sub_cmd);
  install_element (CONFIG_NODE, &no_debug_ospf_kroute_cmd);
