      ...
      for (ALL_LIST_ELEMENTS_RO (adjacencies, node, adj))
        {
          ...
          if (CHECK_FLAG (adj->circuit->flags, ISIS_CIRCUIT_FLAPPED_AFTER_SPF))
            SET_FLAG (rinfo->flag, ISIS_ROUTE_FLAG_KROUTE_RESYNC);
		      ...
		      struct isis_route_info *old, u_char family)
{
  ...
  if (!CHECK_FLAG (old->flag, ISIS_ROUTE_FLAG_KROUTE_SYNCED))
    return 0;

  if (CHECK_FLAG (new->flag, ISIS_ROUTE_FLAG_KROUTE_RESYNC))
    return 0;
...
isis_route_delete (struct prefix *prefix, struct route_table *table)
{
  ...
  if (CHECK_FLAG (rinfo->flag, ISIS_ROUTE_FLAG_KROUTE_SYNCED))
    {
      UNSET_FLAG (rinfo->flag, ISIS_ROUTE_FLAG_ACTIVE);
      if (isis->debugs & DEBUG_RTE_EVENTS)
	zlog_debug ("ISIS-Rte: route delete  %s", buff);
      isis_kroute_route_update (prefix, rinfo);
    }
  ...
  for (rnode = route_top (table); rnode; rnode = route_next (rnode))
      ...
      if (isis->debugs & DEBUG_RTE_EVENTS)
	{
	  ...
	  zlog_debug ("ISIS-Rte (%s): route validate: %s %s %s %s",
		      area->area_tag,
		      (CHECK_FLAG (rinfo->flag, ISIS_ROUTE_FLAG_KROUTE_SYNCED) ?
		      "synced" : "not-synced"),
		      (CHECK_FLAG (rinfo->flag, ISIS_ROUTE_FLAG_KROUTE_RESYNC) ?
		      "resync" : "not-resync"),
		      (CHECK_FLAG (rinfo->flag, ISIS_ROUTE_FLAG_ACTIVE) ?
		      "active" : "inactive"), buff);
	}

      isis_kroute_route_update (&rnode->p, rinfo);
 ...
 * to the RIB with different kroute route types and let RIB handle this? */
static void
