      ...
      && CHECK_FLAG (ri->flags, BGP_INFO_COUNTED))
    {
      ...
      if (ri->peer->pcount[rn->table->afi][rn->table->safi])
      ...
      else
        {
          ...
          zlog_warn ("%s: Please report to Bane bugzilla", __func__);
        }      
    }
...
bgp_process_main (struct work_queue *wq, void *data)
{
      ...
      if (! CHECK_FLAG (old_select->flags, BGP_INFO_ATTR_CHANGED))
        {
	      ...
	      CHECK_FLAG (old_select->flags, BGP_INFO_MULTIPATH_CHG))
            bgp_kroute_announce (p, old_select, bgp, safi);
  ...
  if ((safi == SAFI_UNICAST || safi == SAFI_MULTICAST) && (! bgp->name &&
    ...
    {
      if (new_select 
	  && new_select->type == KROUTE_ROUTE_BGP 
	  && new_select->sub_type == BGP_ROUTE_NORMAL)
	bgp_kroute_announce (p, new_select, bgp, safi);
      else
	{
	  /* Withdraw the route from the kernel. */
	  if (old_select 
	      && old_select->type == KROUTE_ROUTE_BGP
	      && old_select->sub_type == BGP_ROUTE_NORMAL)
	    bgp_kroute_withdraw (p, old_select, safi);
	}
    }
    ...
    for (ain = rn->adj_in; ain; ain = ain->next)
      {
        bgp_update_rsclient (rsclient, afi, safi, ain->attr, ain->peer,
                &rn->p, KROUTE_ROUTE_BGP, BGP_ROUTE_NORMAL, NULL, NULL);
      }
}
	...
	if (ain->peer == peer)
	  {
	    ret = bgp_update (peer, &rn->p, ain->attr, afi, safi,
			      KROUTE_ROUTE_BGP, BGP_ROUTE_NORMAL,
			      NULL, NULL, 1);
	    if (ret < 0)
	      {
		bgp_unlock_node (rn);
...
bgp_cleanup_routes (void)
{
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
	...
	for (ri = rn->info; ri; ri = ri->next)
	  if (CHECK_FLAG (ri->flags, BGP_INFO_SELECTED)
	      && ri->type == KROUTE_ROUTE_BGP 
	      && ri->sub_type == BGP_ROUTE_NORMAL)
	    bgp_kroute_withdraw (&rn->p, ri,SAFI_UNICAST);
...
bgp_nlri_parse (struct peer *peer, struct attr *attr, struct bgp_nlri *packet)
{
  ...
  for (; pnt < lim; pnt += psize)
	...
	ret = bgp_update (peer, &p, attr, packet->afi, packet->safi, 
			  KROUTE_ROUTE_BGP, BGP_ROUTE_NORMAL, NULL, NULL, 0);
      else
	ret = bgp_withdraw (peer, &p, attr, packet->afi, packet->safi, 
			    KROUTE_ROUTE_BGP, BGP_ROUTE_NORMAL, NULL, NULL);
                            ...
                            afi_t afi, safi_t safi)
{
  ...
  for (ri = rn->info; ri; ri = ri->next)
    if (ri->peer == bgp->peer_self && ri->type == KROUTE_ROUTE_BGP
            && ri->sub_type == BGP_ROUTE_STATIC)
      break;
...
/* Configure static BGP network.  When user don't run kroute, static
		      ...
		      struct bgp_info *binfo, afi_t afi, safi_t safi)
{
  ...
  if (attr)
	...
	vty_out (vty, ", aggregated, local");
      else if (binfo->type != KROUTE_ROUTE_BGP)
	vty_out (vty, ", sourced");
      else
...
bgp_distance_apply (struct prefix *p, struct bgp_info *rinfo, struct bgp *bgp)
{
  ...
  if (rn)
      ...
      if (bgp_static->backdoor)
	{
	  if (bgp->distance_local)
	    ...
	    return KROUTE_IBGP_DISTANCE_DEFAULT;
	}
    }
  ...
  if (peer_sort (peer) == BGP_PEER_EBGP)
	...
	return bgp->distance_ebgp;
      return KROUTE_EBGP_DISTANCE_DEFAULT;
    }
  else
    {
...
bgp_config_write_distance (struct vty *vty, struct bgp *bgp)
{
  ...
  if (bgp->distance_ebgp
      && bgp->distance_ibgp
      && bgp->distance_local
      && (bgp->distance_ebgp != KROUTE_EBGP_DISTANCE_DEFAULT
	  || bgp->distance_ibgp != KROUTE_IBGP_DISTANCE_DEFAULT
	  || bgp->distance_local != KROUTE_IBGP_DISTANCE_DEFAULT))
