...
ospf_route_delete (struct route_table *rt)
{
  ...
  for (rn = route_top (rt); rn; rn = route_next (rn))
	...
	if (or->type == OSPF_DESTINATION_NETWORK)
	  ospf_kroute_delete ((struct prefix_ipv4 *) &rn->p,
				       or);
	else if (or->type == OSPF_DESTINATION_DISCARD)
	  ospf_kroute_delete_discard ((struct prefix_ipv4 *) &rn->p);
      }
}
          ...
          if ( (ext_rn = route_node_lookup (external_routes, (struct prefix *)p)) )
            {
              if (ext_rn->info)
                {
                  ospf_kroute_delete (p, ext_rn->info);
  ...
  for (rn = route_top (rt); rn; rn = route_next (rn))
	  ...
	  or->path_type == OSPF_PATH_INTER_AREA)
	{
	  if (or->type == OSPF_DESTINATION_NETWORK)
		...
		ospf_kroute_delete ((struct prefix_ipv4 *) &rn->p, or);
	    }
	  else if (or->type == OSPF_DESTINATION_DISCARD)
	    if (! ospf_route_match_same (cmprt,
					 (struct prefix_ipv4 *) &rn->p, or))
	...
	if (or->type == OSPF_DESTINATION_NETWORK)
	  {
	    if (! ospf_route_match_same (ospf->old_table,
					 (struct prefix_ipv4 *)&rn->p, or))
	      ospf_kroute_add ((struct prefix_ipv4 *) &rn->p, or);
	  }
	else if (or->type == OSPF_DESTINATION_DISCARD)
	  if (! ospf_route_match_same (ospf->old_table,
				       (struct prefix_ipv4 *) &rn->p, or))
	    ospf_kroute_add_discard ((struct prefix_ipv4 *) &rn->p);
      }
}
