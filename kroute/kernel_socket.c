...
#include "kroute/kernel_socket.h"
...
ifm_read (struct if_msghdr *ifm)
{
  ...
  if ((ifp == NULL) || (ifp->ifindex == IFINDEX_INTERNAL))
      ...
      if (!ifnlen)
	{
      ...
      if (ifp == NULL)
        {
	  /* Interface that kroute was not previously aware of, so create. */ 
...
ifam_read (struct ifa_msghdr *ifam)
{
   ...
   * unset (as kept in struct kroute_if.primary_state) and the mangled
...
rtm_read (struct rt_msghdr *rtm)
{
  ...
  if (flags & RTF_PROTO1)
    SET_FLAG (kroute_flags, KROUTE_FLAG_SELFROUTE);

  /* This is persistent route. */
  if (flags & RTF_STATIC)
    SET_FLAG (kroute_flags, KROUTE_FLAG_STATIC);

  /* This is a reject or blackhole route */
  if (flags & RTF_REJECT)
    SET_FLAG (kroute_flags, KROUTE_FLAG_REJECT);
  if (flags & RTF_BLACKHOLE)
    SET_FLAG (kroute_flags, KROUTE_FLAG_BLACKHOLE);
  ...
  if (dest.sa.sa_family == AF_INET)
	...
	p.prefixlen = ip_masklen (mask.sin.sin_addr);
          ...
          case RTM_CHANGE:
            ...
            switch (ret)
            {
              case KROUTE_RIB_NOTFOUND:
                ...
                break;
              case KROUTE_RIB_FOUND_CONNECTED:
              case KROUTE_RIB_FOUND_NOGATE:
                ...
                break;
              case KROUTE_RIB_FOUND_EXACT: /* RIB RR == FIB RR */
          ...
          case RTM_DELETE:
            ...
            switch (ret)
            {
              ...
              case KROUTE_RIB_FOUND_NOGATE:
                ...
                rib_lookup_and_dump (&p);
                break;
              case KROUTE_RIB_NOTFOUND: /* RIB RR == FIB RR */
                zlog_debug ("%s: %s %s/%d: done Ok",
                  __func__, lookup (rtm_type_str, rtm->rtm_type), buf, p.prefixlen);
                rib_lookup_and_dump (&p);
                return;
      ...
      if (rtm->rtm_type == RTM_CHANGE)
        rib_delete_ipv4 (KROUTE_ROUTE_KERNEL, kroute_flags, &p,
                         NULL, 0, 0, SAFI_UNICAST);
      
      if (rtm->rtm_type == RTM_GET 
          || rtm->rtm_type == RTM_ADD
          || rtm->rtm_type == RTM_CHANGE)
	rib_add_ipv4 (KROUTE_ROUTE_KERNEL, kroute_flags, 
		      &p, &gate.sin.sin_addr, NULL, 0, 0, 0, 0, SAFI_UNICAST);
      else
	rib_delete_ipv4 (KROUTE_ROUTE_KERNEL, kroute_flags, 
  ...
  if (dest.sa.sa_family == AF_INET6)
      ...
      if (IN6_IS_ADDR_LINKLOCAL (&gate.sin6.sin6_addr))
	{
       ...
       */
      if (rtm->rtm_type == RTM_CHANGE)
        rib_delete_ipv6 (KROUTE_ROUTE_KERNEL, kroute_flags, &p,
                         NULL, 0, 0, SAFI_UNICAST);
      
      if (rtm->rtm_type == RTM_GET 
          || rtm->rtm_type == RTM_ADD
          || rtm->rtm_type == RTM_CHANGE)
	rib_add_ipv6 (KROUTE_ROUTE_KERNEL, kroute_flags,
		      &p, &gate.sin6.sin6_addr, ifindex, 0, 0, 0, SAFI_UNICAST);
      else
	rib_delete_ipv6 (KROUTE_ROUTE_KERNEL, kroute_flags,
	   ...
	   int metric)
{
  ...
  /* Additional flags. */
  if (kroute_flags & KROUTE_FLAG_BLACKHOLE)
    msg.rtm.rtm_flags |= RTF_BLACKHOLE;
  if (kroute_flags & KROUTE_FLAG_REJECT)
...
#define SOCKADDRSET(X,R) \
  ...
  if (ret != msg.rtm.rtm_msglen) 
    {
      if (errno == EEXIST) 
	return KROUTE_ERR_RTEXIST;
      if (errno == ENETUNREACH)
	return KROUTE_ERR_RTUNREACH;
      if (errno == ESRCH)
	return KROUTE_ERR_RTNOEXIST;
      
      zlog_warn ("%s: write : %s (%d)", __func__, safe_strerror (errno), errno);
      return KROUTE_ERR_KERNEL;
    }
  return KROUTE_ERR_NOERROR;
}
...
kernel_read (struct thread *thread)
{
  ...
  thread_add_read (krouted.master, kernel_read, NULL, sock);
...
routing_socket (void)
{
  ...
  thread_add_read (krouted.master, kernel_read, NULL, routing_sock);
}
