...
size_t
bane_timestamp(int timestamp_precision, char *buf, size_t buflen)
{
  static struct {
    time_t last;
    size_t len;
...
time_print(FILE *fp, struct timestamp_control *ctl)
{
  if (!ctl->already_rendered)
    {
      ctl->len = bane_timestamp(ctl->precision, ctl->buf, sizeof(ctl->buf));
...
open_crashlog(void)
{
#define CRASHLOG_PREFIX "/var/tmp/bane."
...
static const struct kroute_desc_table command_types[] = {
  DESC_ENTRY	(KROUTE_INTERFACE_ADD),
  DESC_ENTRY	(KROUTE_INTERFACE_DELETE),
  DESC_ENTRY	(KROUTE_INTERFACE_ADDRESS_ADD),
  DESC_ENTRY	(KROUTE_INTERFACE_ADDRESS_DELETE),
  DESC_ENTRY	(KROUTE_INTERFACE_UP),
  DESC_ENTRY	(KROUTE_INTERFACE_DOWN),
  DESC_ENTRY	(KROUTE_IPV4_ROUTE_ADD),
  DESC_ENTRY	(KROUTE_IPV4_ROUTE_DELETE),
  DESC_ENTRY	(KROUTE_IPV6_ROUTE_ADD),
  DESC_ENTRY	(KROUTE_IPV6_ROUTE_DELETE),
  DESC_ENTRY	(KROUTE_REDISTRIBUTE_ADD),
  DESC_ENTRY	(KROUTE_REDISTRIBUTE_DELETE),
  DESC_ENTRY	(KROUTE_REDISTRIBUTE_DEFAULT_ADD),
  DESC_ENTRY	(KROUTE_REDISTRIBUTE_DEFAULT_DELETE),
  DESC_ENTRY	(KROUTE_IPV4_NEXTHOP_LOOKUP),
  DESC_ENTRY	(KROUTE_IPV6_NEXTHOP_LOOKUP),
  DESC_ENTRY	(KROUTE_IPV4_IMPORT_LOOKUP),
  DESC_ENTRY	(KROUTE_IPV6_IMPORT_LOOKUP),
  DESC_ENTRY	(KROUTE_INTERFACE_RENAME),
  DESC_ENTRY	(KROUTE_ROUTER_ID_ADD),
  DESC_ENTRY	(KROUTE_ROUTER_ID_DELETE),
  DESC_ENTRY	(KROUTE_ROUTER_ID_UPDATE),
  DESC_ENTRY	(KROUTE_HELLO),
};
#undef DESC_ENTRY

static const struct kroute_desc_table unknown = { 0, "unknown", '?' };

static const struct kroute_desc_table *
zroute_lookup(u_int zroute)
{
  ...
  if (zroute >= sizeof(route_types)/sizeof(route_types[0]))
    {
      zlog_err("unknown kroute route type: %u", zroute);
      return &unknown;
    }
...
const char *
kroute_route_string(u_int zroute)
{
  return zroute_lookup(zroute)->string;
}

char
kroute_route_char(u_int zroute)
{
...
proto_redistnum(int afi, const char *s)
{
  ...
  if (afi == AFI_IP)
    {
      if (strncmp (s, "k", 1) == 0)
	return KROUTE_ROUTE_KERNEL;
      else if (strncmp (s, "c", 1) == 0)
	return KROUTE_ROUTE_CONNECT;
      else if (strncmp (s, "s", 1) == 0)
	return KROUTE_ROUTE_STATIC;
      else if (strncmp (s, "r", 1) == 0)
	return KROUTE_ROUTE_RIP;
      else if (strncmp (s, "o", 1) == 0)
	return KROUTE_ROUTE_OSPF;
      else if (strncmp (s, "i", 1) == 0)
	return KROUTE_ROUTE_ISIS;
      else if (strncmp (s, "bg", 2) == 0)
	return KROUTE_ROUTE_BGP;
      else if (strncmp (s, "ba", 2) == 0)
	return KROUTE_ROUTE_BABEL;
    }
  if (afi == AFI_IP6)
    {
      if (strncmp (s, "k", 1) == 0)
	...
	return KROUTE_ROUTE_RIPNG;
      else if (strncmp (s, "o", 1) == 0)
	return KROUTE_ROUTE_OSPF6;
      else if (strncmp (s, "i", 1) == 0)
	return KROUTE_ROUTE_ISIS;
      else if (strncmp (s, "bg", 2) == 0)
	return KROUTE_ROUTE_BGP;
