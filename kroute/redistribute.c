...
/* master kroute server structure */
extern struct kroute_t krouted;

int
kroute_check_addr (struct prefix *p)
{
  if (p->family == AF_INET)
    {
      u_int32_t addr;
...
static void
kroute_redistribute_default (struct zserv *client)
{
  ...
  if (table)
      ...
      if (rn)
	{
	  for (newrib = rn->info; newrib; newrib = newrib->next)
	    if (CHECK_FLAG (newrib->flags, KROUTE_FLAG_SELECTED)
		&& newrib->distance != DISTANCE_INFINITY)
	      zsend_route_multipath (KROUTE_IPV4_ROUTE_ADD, client, &rn->p, newrib);
  ...
  if (table)
      ...
      if (rn)
	{
	  for (newrib = rn->info; newrib; newrib = newrib->next)
	    if (CHECK_FLAG (newrib->flags, KROUTE_FLAG_SELECTED)
		&& newrib->distance != DISTANCE_INFINITY)
	      zsend_route_multipath (KROUTE_IPV6_ROUTE_ADD, client, &rn->p, newrib);
	  route_unlock_node (rn);
	}
    }
#endif /* HAVE_IPV6 */
...
/* Redistribute routes. */
static void
kroute_redistribute (struct zserv *client, int type)
{
  struct rib *newrib;
  struct route_table *table;
  struct route_node *rn;
...
redistribute_add (struct prefix *p, struct rib *rib)
{
  ...
  for (ALL_LIST_ELEMENTS (krouted.client_list, node, nnode, client))
    {
      if (is_default (p))
        {
          if (client->redist_default || client->redist[rib->type])
            {
              if (p->family == AF_INET)
                zsend_route_multipath (KROUTE_IPV4_ROUTE_ADD, client, p, rib);
#ifdef HAVE_IPV6
              if (p->family == AF_INET6)
                zsend_route_multipath (KROUTE_IPV6_ROUTE_ADD, client, p, rib);
#endif /* HAVE_IPV6 */	  
	    }
        }
  ...
  for (ALL_LIST_ELEMENTS (krouted.client_list, node, nnode, client))
      ...
      if (is_default (p))
	{
	  if (client->redist_default || client->redist[rib->type])
	    {
	      if (p->family == AF_INET)
		zsend_route_multipath (KROUTE_IPV4_ROUTE_DELETE, client, p,
				       rib);
#ifdef HAVE_IPV6
	      if (p->family == AF_INET6)
		zsend_route_multipath (KROUTE_IPV6_ROUTE_DELETE, client, p,
				       rib);
#endif /* HAVE_IPV6 */
	    }
	}
	...
	{
	  if (p->family == AF_INET)
	    zsend_route_multipath (KROUTE_IPV4_ROUTE_DELETE, client, p, rib);
#ifdef HAVE_IPV6
	  if (p->family == AF_INET6)
	    zsend_route_multipath (KROUTE_IPV6_ROUTE_DELETE, client, p, rib);
#endif /* HAVE_IPV6 */
	}
    }
}
...
void
kroute_redistribute_add (int command, struct zserv *client, int length)
{
  int type;

  type = stream_getc (client->ibuf);

  if (type == 0 || type >= KROUTE_ROUTE_MAX)
    return;

  if (! client->redist[type])
    {
      client->redist[type] = 1;
      kroute_redistribute (client, type);
    }
}

void
kroute_redistribute_delete (int command, struct zserv *client, int length)
{
  int type;
...
void
kroute_redistribute_default_add (int command, struct zserv *client, int length)
{
  client->redist_default = 1;
  kroute_redistribute_default (client);
}     

void
kroute_redistribute_default_delete (int command, struct zserv *client,
				   int length)
{
...
kroute_interface_up_update (struct interface *ifp)
{
  ...
  for (ALL_LIST_ELEMENTS (krouted.client_list, node, nnode, client))
    zsend_interface_update (KROUTE_INTERFACE_UP, client, ifp);
}
...
kroute_interface_down_update (struct interface *ifp)
{
  ...
  for (ALL_LIST_ELEMENTS (krouted.client_list, node, nnode, client))
    zsend_interface_update (KROUTE_INTERFACE_DOWN, client, ifp);
}
				    ...
				    struct connected *ifc)
{
  ...
  for (ALL_LIST_ELEMENTS (krouted.client_list, node, nnode, client))
    if (client->ifinfo && CHECK_FLAG (ifc->conf, KROUTE_IFC_REAL))
      zsend_interface_address (KROUTE_INTERFACE_ADDRESS_ADD, client, ifp, ifc);
}
				       ...
				       struct connected *ifc)
{
  ...
  for (ALL_LIST_ELEMENTS (krouted.client_list, node, nnode, client))
    if (client->ifinfo && CHECK_FLAG (ifc->conf, KROUTE_IFC_REAL))
      zsend_interface_address (KROUTE_INTERFACE_ADDRESS_DELETE, client, ifp, ifc);
}
