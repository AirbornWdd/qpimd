...
zclient_new ()
{
  ...
  zclient->ibuf = stream_new (KROUTE_MAX_PACKET_SIZ);
  zclient->obuf = stream_new (KROUTE_MAX_PACKET_SIZ);
...
/* Initialize kroute client.  Argument redist_default is unwanted
...
zclient_init (struct zclient *zclient, int redist_default)
{
  ...
  /* Enable kroute client connection by default. */
  zclient->enable = 1;
...
/* Make socket to kroute daemon. Return kroute socket. */
static int
zclient_socket(void)
{
  int sock;
  ...
  serv.sin_port = htons (KROUTE_PORT);
...
#else
  zclient->sock = zclient_socket_un (zclient_serv_path ? zclient_serv_path : KROUTE_SERV_PATH);
#endif
  return zclient->sock;
}
...
/* Send simple Kroute message. */
static int
kroute_message_send (struct zclient *zclient, int command)
{
  struct stream *s;
  ...
  /* Send very simple command only Kroute message. */
...
static int
kroute_hello_send (struct zclient *zclient)
{
  struct stream *s;
  ...
  if (zclient->redist_default)
    {
      ...
      zclient_create_header (s, KROUTE_HELLO);
...
zclient_start (struct zclient *zclient)
{
  ...
  /* If already connected to the kroute. */
  if (zclient->sock >= 0)
    return 0;
  ...
  kroute_hello_send (zclient);

  /* We need router-id information. */
  kroute_message_send (zclient, KROUTE_ROUTER_ID_ADD);

  /* We need interface information. */
  kroute_message_send (zclient, KROUTE_INTERFACE_ADD);

  /* Flush all redistribute request. */
  for (i = 0; i < KROUTE_ROUTE_MAX; i++)
    if (i != zclient->redist_default && zclient->redist[i])
      kroute_redistribute_send (KROUTE_REDISTRIBUTE_ADD, zclient, i);

  /* If default information is needed. */
  if (zclient->default_information)
    kroute_message_send (zclient, KROUTE_REDISTRIBUTE_DEFAULT_ADD);
...

  ...
  * a message to kroute server for a route that needs to be
  ...
  * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  * | KROUTE Flags   | Message Flags | Prefix length |
  * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  *
  * +-+-+-+-+-+-+-+-+
  * | Nexthop Type  |  Set to one of KROUTE_NEXTHOP_*
  * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  * |       IPv4 Nexthop address or Interface Index number          |
  * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  *
  * Alternatively, if the flags field has KROUTE_FLAG_BLACKHOLE or
  * KROUTE_FLAG_REJECT is set then Nexthop count is set to 1, then _no_ 
                 ...
                 struct zapi_ipv4 *api)
{
  ...
  if (CHECK_FLAG (api->message, ZAPI_MESSAGE_NEXTHOP))
    {
      if (CHECK_FLAG (api->flags, KROUTE_FLAG_BLACKHOLE))
        {
          stream_putc (s, 1);
          stream_putc (s, KROUTE_NEXTHOP_BLACKHOLE);
      ...
      for (i = 0; i < api->nexthop_num; i++)
        {
          stream_putc (s, KROUTE_NEXTHOP_IPV4);
          stream_put_in_addr (s, api->nexthop[i]);
        }
      for (i = 0; i < api->ifindex_num; i++)
        {
          stream_putc (s, KROUTE_NEXTHOP_IFINDEX);
  ...
  if (CHECK_FLAG (api->message, ZAPI_MESSAGE_NEXTHOP))
    {
      ...
      for (i = 0; i < api->nexthop_num; i++)
	{
	  stream_putc (s, KROUTE_NEXTHOP_IPV6);
...
/* 
 * send a KROUTE_REDISTRIBUTE_ADD or KROUTE_REDISTRIBUTE_DELETE
 * for the route type (KROUTE_ROUTE_KERNEL etc.). The kroute server will
 * then set/unset redist[type] in the client handle (a struct zserv) for the 
 * sending client
 */
int
kroute_redistribute_send (int command, struct zclient *zclient, int type)
{
  struct stream *s;
...
/* Router-id update from kroute daemon. */
void
kroute_router_id_update_read (struct stream *s, struct prefix *rid)
{
  int plen;
...
/* Interface addition from kroute daemon. */
/*  
 * The format of the message sent with type KROUTE_INTERFACE_ADD or
 * KROUTE_INTERFACE_DELETE from kroute to the client is:
 *     0                   1                   2                   3
...
struct interface *
kroute_interface_add_read (struct stream *s)
{
  ...
  kroute_interface_if_set_value (s, ifp);

  return ifp;
}

/* 
 * Read interface up/down msg (KROUTE_INTERFACE_UP/KROUTE_INTERFACE_DOWN)
 * from kroute server.  The format of this message is the same as
 * that sent for KROUTE_INTERFACE_ADD/KROUTE_INTERFACE_DELETE (see
 * comments for kroute_interface_add_read), except that no sockaddr_dl
 * is sent at the tail of the message.
 */
struct interface *
kroute_interface_state_read (struct stream *s)
{
  struct interface *ifp;
  char ifname_tmp[INTERFACE_NAMSIZ];
 ...
 *  0 1 2 3 4 5 6 7
 * +-+-+-+-+-+-+-+-+
 * |   type        |  KROUTE_INTERFACE_ADDRESS_ADD or
 * +-+-+-+-+-+-+-+-+  KROUTE_INTERFACE_ADDRES_DELETE
 * |               |
 * +               +
 * |   ifindex     |
 * +               +
...
void
kroute_interface_if_set_value (struct stream *s, struct interface *ifp)
{
...
struct connected *
kroute_interface_address_read (int type, struct stream *s)
{
  ...
  if (ifp == NULL)
    {
      zlog_warn ("kroute_interface_address_read(%s): "
                 "Can't find interface by ifindex: %d ",
                 (type == KROUTE_INTERFACE_ADDRESS_ADD? "ADD" : "DELETE"),
                 ifindex);
      return NULL;
    }
  ...
  if (type == KROUTE_INTERFACE_ADDRESS_ADD) 
    {
    ...
    {
      assert (type == KROUTE_INTERFACE_ADDRESS_DELETE);
...

/* Kroute client message read function. */
static int
zclient_read (struct thread *thread)
{
  ...
  /* Get socket to kroute. */
  zclient = THREAD_ARG (thread);
  zclient->t_read = NULL;

  /* Read kroute header (if we don't have it already). */
  if ((already = stream_get_endp(zclient->ibuf)) < KROUTE_HEADER_SIZE)
    {
      ssize_t nbyte;
  ...
  if (length < KROUTE_HEADER_SIZE) 
    {
      zlog_err("%s: socket %d message length %u is less than %d ",
	       __func__, zclient->sock, length, KROUTE_HEADER_SIZE);
      return zclient_failed(zclient);
    }
...
zclient_redistribute (int command, struct zclient *zclient, int type)
{
  ...
  if (command == KROUTE_REDISTRIBUTE_ADD) 
    {
  ...
  if (zclient->sock > 0)
    kroute_redistribute_send (command, zclient, type);
}
...
zclient_redistribute_default (int command, struct zclient *zclient)
{
  ...
  if (command == KROUTE_REDISTRIBUTE_DEFAULT_ADD)
    {
      if (zclient->default_information)
        return;
  ...
  if (zclient->sock > 0)
    kroute_message_send (zclient, command);
}
...
zclient_serv_path_set (char *path)
{
  ...
  if (stat(path, &sb) == -1)
    {
      zlog_warn ("%s: kroute socket `%s' does not exist", __func__, path);
      return;
    }
