...
{
  { .fd = -1, .name = "kroute", .flag = VTYSH_KROUTE, .path = KROUTE_VTYSH_PATH},
...
/* Using integrated config from Bane.conf. Default is no. */
...
DEFUNSH (VTYSH_KROUTE,
	 vtysh_exit_kroute,
	 vtysh_exit_kroute_cmd,
	 "exit",
	 "Exit current mode and down to previous mode\n")
{
  return vtysh_exit (vty);
...
ALIAS (vtysh_exit_kroute,
       vtysh_quit_kroute_cmd,
       "quit",
       "Exit current mode and down to previous mode\n")
...
DEFSH (VTYSH_KROUTE|VTYSH_RIPD|VTYSH_RIPNGD|VTYSH_OSPFD|VTYSH_OSPF6D,
 ...
 * and isisd. */
DEFSH (VTYSH_KROUTE|VTYSH_RIPD|VTYSH_OSPFD,
       interface_desc_cmd,
       "description .LINE",
       ...
       "Write configuration to the file (same as write file)\n")
{
  ...
  /* If integrated Bane.conf explicitely set. */
...
/* #endif */
  ...
  vtysh_install_default (KROUTE_NODE);
