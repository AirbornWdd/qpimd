...
netlink_interface_addr (struct sockaddr_nl *snl, struct nlmsghdr *h)
{
  ...
  if (IS_KROUTE_DEBUG_KERNEL)    /* remove this line to see initial ifcfg */
    {
      char buf[BUFSIZ];
...
netlink_routing_table (struct sockaddr_nl *snl, struct nlmsghdr *h)
{
  ...
  if (table != RT_TABLE_MAIN && table != krouted.rtm_table_default)
    return 0;
#endif
  ...
  /* Route which inserted by Kroute. */
  if (rtm->rtm_protocol == RTPROT_ZEBRA)
    flags |= KROUTE_FLAG_SELFROUTE;
  ...
  if (rtm->rtm_family == AF_INET)
    {
      ...
      if (!tb[RTA_MULTIPATH])
          rib_add_ipv4 (KROUTE_ROUTE_KERNEL, flags, &p, gate, src, index,
  ...
  if (rtm->rtm_family == AF_INET6)
    {
      ...
      rib_add_ipv6 (KROUTE_ROUTE_KERNEL, flags, &p, gate, index, table,
  ...
  if (rtm->rtm_family == AF_INET)
    {
      ...
      if (h->nlmsg_type == RTM_NEWROUTE)
        {
          if (!tb[RTA_MULTIPATH])
            rib_add_ipv4 (KROUTE_ROUTE_KERNEL, 0, &p, gate, src, index, table,
                          metric, 0, SAFI_UNICAST);
          else
            {
              /* This is a multipath route */
        ...
        rib_delete_ipv4 (KROUTE_ROUTE_KERNEL, 0, &p, gate, index, table, SAFI_UNICAST);
    }
  ...
  if (rtm->rtm_family == AF_INET6)
    {
      ...
      if (h->nlmsg_type == RTM_NEWROUTE)
        rib_add_ipv6 (KROUTE_ROUTE_KERNEL, 0, &p, gate, index, table, metric, 0, SAFI_UNICAST);
      else
        rib_delete_ipv6 (KROUTE_ROUTE_KERNEL, 0, &p, gate, index, table, SAFI_UNICAST);
    }
#endif /* HAVE_IPV6 */
...
netlink_route (int cmd, int family, void *dest, int length, void *gate,
               int index, int kroute_flags, int table)
{
  int ret;
  int bytelen;
  struct sockaddr_nl snl;
  ...
  if ((kroute_flags & KROUTE_FLAG_BLACKHOLE)
      || (kroute_flags & KROUTE_FLAG_REJECT))
    discard = 1;
  else
    discard = 0;
      ...
      if (discard)
        {
          ...
          else if (kroute_flags & KROUTE_FLAG_REJECT)
                         ...
                         int family)
{
  ...
  if ((rib->flags & KROUTE_FLAG_BLACKHOLE) || (rib->flags & KROUTE_FLAG_REJECT))
    discard = 1;
  else
    discard = 0;
  ...
  if (cmd == RTM_NEWROUTE)
    {
      if (discard)
        {
          if (rib->flags & KROUTE_FLAG_BLACKHOLE)
            req.r.rtm_type = RTN_BLACKHOLE;
          else if (rib->flags & KROUTE_FLAG_REJECT)
            req.r.rtm_type = RTN_UNREACHABLE;
          else
            assert (RTN_BLACKHOLE != RTN_UNREACHABLE);  /* false */
        }
...
kernel_read (struct thread *thread)
{
  ...
  thread_add_read (krouted.master, kernel_read, NULL, netlink.sock);
