 ...
 * Copyright (C) 1998, 2000 Kunihiro Ishiguro <kunihiro@kroute.org>
 *
...
static void
bane_gettimeofday_relative_adjust (void)
{
  struct timeval diff;
...
static int
bane_gettimeofday (struct timeval *tv)
{
  int ret;
...
static int
bane_get_relative (struct timeval *tv)
{
  int ret;
  ...
  if (!(ret = bane_gettimeofday (&recent_time)))
    bane_gettimeofday_relative_adjust();
#endif /* HAVE_CLOCK_MONOTONIC */
 ...
 */
static void
bane_real_stabilised (struct timeval *tv)
{
...
/* Exported Bane timestamp function.
 * Modelled on POSIX clock_gettime.
 */
int
bane_gettime (enum bane_clkid clkid, struct timeval *tv)
{
  switch (clkid)
    {
      case BANE_CLK_REALTIME:
        return bane_gettimeofday (tv);
      case BANE_CLK_MONOTONIC:
        return bane_get_relative (tv);
      case BANE_CLK_REALTIME_STABILISED:
        bane_real_stabilised (tv);
        return 0;
      default:
        errno = EINVAL;
        return -1;
