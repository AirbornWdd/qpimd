...
ospf_router_id_update (struct ospf *ospf)
{
  ...
  /* Select the router ID based on these priorities:
	  ...
	  disruptive.
       3. Last choice: just go with whatever the kroute daemon recommends.
  */
	  ...
	  for (type = 0; type < KROUTE_ROUTE_MAX; type++)
	    if (ospf->external_origin & (1 << type))
	      thread_add_event (master, ospf_external_lsa_originate_timer,
				ospf, type);
	  /* Originate Deafult. */
	  if (ospf->external_origin & (1 << KROUTE_ROUTE_MAX))
...
ospf_new (void)
{
  ...
  new->lsa_refresher_started = bane_time (NULL);
...
ospf_finish_final (struct ospf *ospf)
{
  ...
  for (i = KROUTE_ROUTE_SYSTEM; i <= KROUTE_ROUTE_MAX; i++)
		  ...
		  struct in_addr area_id)
{
  ...
  if (ospf_is_type_redistributed (KROUTE_ROUTE_CONNECT))
    if (EXTERNAL_INFO (KROUTE_ROUTE_CONNECT))
      for (rn = route_top (EXTERNAL_INFO (KROUTE_ROUTE_CONNECT));
	   rn; rn = route_next (rn))
	if ((ei = rn->info) != NULL)
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, cnode, co))
    {
      ...
      if (CHECK_FLAG(co->flags,KROUTE_IFA_SECONDARY))
        continue;
...
ospf_timers_refresh_set (struct ospf *ospf, int interval)
{
  ...
  time_left = ospf->lsa_refresh_interval -
    (bane_time (NULL) - ospf->lsa_refresher_started);
