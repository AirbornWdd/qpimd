...
extern int rtm_write (int message, union sockunion *dest,
                      ...
                      unsigned int index, int kroute_flags, int metric);
...
/* Interface between kroute message and rtm message. */
static int
...

{
  ...
  for (nexthop = rib->nexthop; nexthop; nexthop = nexthop->next)
	      ...
	      ))
	{
	  ...
	  error = rtm_write (cmd,
           ...
           switch (error)
           {
              ...
              */
             case KROUTE_ERR_RTNOEXIST:
             case KROUTE_ERR_RTUNREACH:
             default:
   ...
   if (nexthop_num == 0 && IS_KROUTE_DEBUG_KERNEL)
