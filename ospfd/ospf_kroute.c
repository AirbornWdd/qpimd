/*
 * Kroute connect library for OSPFd
...
#include "ospfd/ospf_kroute.h"
#ifdef HAVE_SNMP
#include "ospfd/ospf_snmp.h"
#endif /* HAVE_SNMP */

/* Kroute structure to hold current status. */
...
static int
ospf_router_id_update_kroute (int command, struct zclient *zclient,
			     kroute_size_t length)
{
  ...
  if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
    {
      char buf[128];
...
static int
ospf_interface_add (int command, struct zclient *zclient, kroute_size_t length)
{
  ...
  if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
    zlog_debug ("Kroute: interface add %s index %d flags %llx metric %d mtu %d",
                       ...
                       kroute_size_t length)
{
  ...
  if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
    zlog_debug
      ("Kroute: interface delete %s index %d flags %lld metric %d mtu %d",
...
kroute_interface_if_lookup (struct stream *s)
{
                         ...
                         kroute_size_t length)
{
  ...
  ifp = kroute_interface_if_lookup (zclient->ibuf);
  ...
  if (if_is_operative (ifp))
    {
      ...
      kroute_interface_if_set_value (zclient->ibuf, ifp);

      if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
        zlog_debug ("Kroute: Interface[%s] state update.", ifp->name);

      if (if_tmp.bandwidth != ifp->bandwidth)
        {
          if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
            zlog_debug ("Kroute: Interface[%s] bandwidth change %d -> %d.",
      ...
      if (if_tmp.mtu != ifp->mtu)
        {
          if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
            zlog_debug ("Kroute: Interface[%s] MTU change %u -> %u.",
  ...
  if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
    zlog_debug ("Kroute: Interface[%s] state change to up.", ifp->name);
                           ...
                           kroute_size_t length)
{
  ...
  if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
    zlog_debug ("Kroute: Interface[%s] state change to down.", ifp->name);
                            ...
                            kroute_size_t length)
{
  ...
  c = kroute_interface_address_read (command, zclient->ibuf);
  ...
  if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
    {
      ...
      zlog_debug("Kroute: interface %s address add %s", c->ifp->name, buf);
    }
  ...
  if (IS_DEBUG_OSPF (kroute, KROUTE_INTERFACE))
    {
      char buf[128];
      prefix2str(c->address, buf, sizeof(buf));
      zlog_debug("Kroute: interface %s address delete %s", c->ifp->name, buf);
    }
...
void
ospf_kroute_add (struct prefix_ipv4 *p, struct ospf_route *or)
{
  u_char message;
  u_char distance;
  u_char flags;
  ...
  if (zclient->redist[KROUTE_ROUTE_OSPF])
    {
      message = 0;
      flags = 0;
      ...
      zclient_create_header (s, KROUTE_IPV4_ROUTE_ADD);
      stream_putc (s, KROUTE_ROUTE_OSPF);
      stream_putc (s, flags);
      ...
      for (ALL_LIST_ELEMENTS_RO (or->paths, node, path))
        {
          ...
          if (IS_DEBUG_OSPF (kroute, KROUTE_REDISTRIBUTE))
            {
	      char buf[2][INET_ADDRSTRLEN];
	      zlog_debug("Kroute: Route add %s/%d nexthop %s",
...
void
ospf_kroute_delete (struct prefix_ipv4 *p, struct ospf_route *or)
{
      ...
      for (ALL_LIST_ELEMENTS (or->paths, node, nnode, path))
        {
          ...
          else if ( IS_DEBUG_OSPF(kroute,KROUTE_REDISTRIBUTE) )
            {
              zlog_debug("Kroute: no ifp %s %d",
                         inet_ntoa(p->prefix),
                         p->prefixlen);
            }

          zapi_ipv4_route (KROUTE_IPV4_ROUTE_DELETE, zclient, p, &api);

          if (IS_DEBUG_OSPF (kroute, KROUTE_REDISTRIBUTE) && api.nexthop_num)
            {
	      char buf[2][INET_ADDRSTRLEN];
	      zlog_debug("Kroute: Route delete %s/%d nexthop %s",
			 inet_ntop(AF_INET, &p->prefix, buf[0], sizeof(buf[0])),
			 p->prefixlen,
			 inet_ntop(AF_INET, *api.nexthop,
				   buf[1], sizeof(buf[1])));
            }
          if (IS_DEBUG_OSPF (kroute, KROUTE_REDISTRIBUTE) && api.ifindex_num)
            {
              zlog_debug ("Kroute: Route delete %s/%d ifindex %d",
                         inet_ntoa (p->prefix),
...
void
ospf_kroute_add_discard (struct prefix_ipv4 *p)
{
  struct zapi_ipv4 api;
  ...
  if (zclient->redist[KROUTE_ROUTE_OSPF])
    {
      api.type = KROUTE_ROUTE_OSPF;
      api.flags = KROUTE_FLAG_BLACKHOLE;
      api.message = 0;
      api.safi = SAFI_UNICAST;
      SET_FLAG (api.message, ZAPI_MESSAGE_NEXTHOP);
      api.nexthop_num = 0;
      ...
      zapi_ipv4_route (KROUTE_IPV4_ROUTE_ADD, zclient, p, &api);

      if (IS_DEBUG_OSPF (kroute, KROUTE_REDISTRIBUTE))
        zlog_debug ("Kroute: Route add discard %s/%d",
                   inet_ntoa (p->prefix), p->prefixlen);
    }
}

void
ospf_kroute_delete_discard (struct prefix_ipv4 *p)
{
  struct zapi_ipv4 api;
  ...
  if (zclient->redist[KROUTE_ROUTE_OSPF])
    {
      ...
      if (IS_DEBUG_OSPF (kroute, KROUTE_REDISTRIBUTE))
        zlog_debug ("Kroute: Route delete discard %s/%d",
                   inet_ntoa (p->prefix), p->prefixlen);
...
ospf_redistribute_unset (struct ospf *ospf, int type)
{
  ...
  zclient_redistribute (KROUTE_REDISTRIBUTE_DELETE, zclient, type);
                               ...
                               int mtype, int mvalue)
{
  ...
  zclient_redistribute_default (KROUTE_REDISTRIBUTE_DEFAULT_ADD, zclient);
...
ospf_redistribute_default_unset (struct ospf *ospf)
{
  ...
  zclient_redistribute_default (KROUTE_REDISTRIBUTE_DEFAULT_DELETE, zclient);
...
static int
ospf_kroute_read_ipv4 (int command, struct zclient *zclient,
                      kroute_size_t length)
{
  struct stream *s;
  struct zapi_ipv4 api;
  ...
  if (command == KROUTE_IPV4_ROUTE_ADD)
    {
       ...
       *     || CHECK_FLAG (api.flags, KROUTE_FLAG_REJECT))
       * return 0;
       */
              ...
              if (is_prefix_default (&p))
              ...
              else
                {
                  ...
                  if (!current)
                    ...
                    zlog_warn ("ospf_kroute_read_ipv4() : %s already exists",
                               inet_ntoa (p.prefix));
                }
            }
        }
    }
  else                          /* if (command == KROUTE_IPV4_ROUTE_DELETE) */
    {
...
ospf_distribute_list_update_timer (struct thread *thread)
{
  ...
  zlog_info ("Kroute[Redistribute]: distribute-list update timer fired!");

  /* foreach all external info. */
  for (type = 0; type <= KROUTE_ROUTE_MAX; type++)
    {
  ...
  for (type = 0; type <= KROUTE_ROUTE_MAX; type++)
    {
      ...
      /* There is place for route-map for default-information (KROUTE_ROUTE_MAX),
       * but no distribute list. */
      if (type == KROUTE_ROUTE_MAX)
	break;
...
ospf_kroute_init ()
{
  ...
  zclient = zclient_new ();
  zclient_init (zclient, KROUTE_ROUTE_OSPF);
  zclient->router_id_update = ospf_router_id_update_kroute;
  ...
  zclient->ipv4_route_add = ospf_kroute_read_ipv4;
  zclient->ipv4_route_delete = ospf_kroute_read_ipv4;
