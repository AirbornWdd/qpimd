...
{  
  int key;
  int distance;
} route_info[KROUTE_ROUTE_MAX] =
{
  [KROUTE_ROUTE_SYSTEM]  = {KROUTE_ROUTE_SYSTEM,    0},
  [KROUTE_ROUTE_KERNEL]  = {KROUTE_ROUTE_KERNEL,    0},
  [KROUTE_ROUTE_CONNECT] = {KROUTE_ROUTE_CONNECT,   0},
  [KROUTE_ROUTE_STATIC]  = {KROUTE_ROUTE_STATIC,    1},
  [KROUTE_ROUTE_RIP]     = {KROUTE_ROUTE_RIP,     120},
  [KROUTE_ROUTE_RIPNG]   = {KROUTE_ROUTE_RIPNG,   120},
  [KROUTE_ROUTE_OSPF]    = {KROUTE_ROUTE_OSPF,    110},
  [KROUTE_ROUTE_OSPF6]   = {KROUTE_ROUTE_OSPF6,   110},
  [KROUTE_ROUTE_ISIS]    = {KROUTE_ROUTE_ISIS,    115},
  [KROUTE_ROUTE_BGP]     = {KROUTE_ROUTE_BGP,      20  /* IBGP is 200. */},
  [KROUTE_ROUTE_BABEL]   = {KROUTE_ROUTE_BABEL,    95},
...
nexthop_blackhole_add (struct rib *rib)
{
  ...
  SET_FLAG (rib->flags, KROUTE_FLAG_BLACKHOLE);
  ...
  while (rn)
      ...
      for (match = rn->info; match; match = match->next)
	{
	  if (CHECK_FLAG (match->status, RIB_ENTRY_REMOVED))
	    continue;
	  if (CHECK_FLAG (match->flags, KROUTE_FLAG_SELECTED))
	    break;
	}
	...
	{
	  if (match->type == KROUTE_ROUTE_CONNECT)
	    {
	  ...
	  else if (CHECK_FLAG (rib->flags, KROUTE_FLAG_INTERNAL))
	    {
...
rib_lookup_ipv4 (struct prefix_ipv4 *p)
{
  ...
  if (! match || match->type == KROUTE_ROUTE_BGP)
    return NULL;
 ...
 * the whole RIB and has KROUTE_FLAG_SELECTED set.
 *
 * Return values:
 * -1: error
 * 0: exact match found
...
rib_lookup_ipv4_route (struct prefix_ipv4 *p, union sockunion * qgate)
{
  ...
  table = vrf_table (AFI_IP, SAFI_UNICAST, 0);
  if (! table)
    return KROUTE_RIB_LOOKUP_ERROR;
  ...
  if (match->type == KROUTE_ROUTE_CONNECT)
    return KROUTE_RIB_FOUND_CONNECTED;
  ...
  for (nexthop = match->nexthop; nexthop; nexthop = nexthop->next)
    if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB))
    {
      ...
      else
      {
        ...
        return KROUTE_RIB_FOUND_NOGATE;
      }
    }
...
#define RIB_SYSTEM_ROUTE(R) \
        ((R)->type == KROUTE_ROUTE_KERNEL || (R)->type == KROUTE_ROUTE_CONNECT)
		      ...
		      struct nexthop *nexthop, int set)
{
  ...
  extern char *proto_rm[AFI_MAX][KROUTE_ROUTE_MAX+1];
  ...
  rmap = 0;
  if (rib->type >= 0 && rib->type < KROUTE_ROUTE_MAX &&
        	proto_rm[family][rib->type])
    rmap = route_map_lookup_by_name (proto_rm[family][rib->type]);
  if (!rmap && proto_rm[family][KROUTE_ROUTE_MAX])
    rmap = route_map_lookup_by_name (proto_rm[family][KROUTE_ROUTE_MAX]);
  if (rmap) {
      ret = route_map_apply(rmap, &rn->p, RMAP_KROUTE, nexthop);
  }
 ...
 * is flagged with KROUTE_FLAG_CHANGED. The 4th 'set' argument is
...
nexthop_active_update (struct route_node *rn, struct rib *rib, int set)
{
  ...
  UNSET_FLAG (rib->flags, KROUTE_FLAG_CHANGED);
  ...
  for (nexthop = rib->nexthop; nexthop; nexthop = nexthop->next)
  {
	...
	prev_index != nexthop->ifindex)
      SET_FLAG (rib->flags, KROUTE_FLAG_CHANGED);
  }
...
{
  if (CHECK_FLAG (rib->flags, KROUTE_FLAG_SELECTED))
    {
      redistribute_delete (&rn->p, rib);
      if (! RIB_SYSTEM_ROUTE (rib))
	rib_uninstall_kernel (rn, rib);
      UNSET_FLAG (rib->flags, KROUTE_FLAG_SELECTED);
    }
}
...
rib_process (struct route_node *rn)
{
  ...
  if (IS_KROUTE_DEBUG_RIB || IS_KROUTE_DEBUG_RIB_Q)
  ...
  for (rib = rn->info; rib; rib = next)
    {
       ...
       */
      if (rib->type == KROUTE_ROUTE_CONNECT)
        {
          if (select->type != KROUTE_ROUTE_CONNECT
              || rib->metric <= select->metric)
            select = rib;
          continue;
        }
      else if (select->type == KROUTE_ROUTE_CONNECT)
        continue;
  ...
  if (select && select == fib)
    {
      ...
      if (CHECK_FLAG (select->flags, KROUTE_FLAG_CHANGED))
        {
  ...
  if (fib)
	...
	rib_uninstall_kernel (rn, fib);
      UNSET_FLAG (fib->flags, KROUTE_FLAG_SELECTED);
  ...
  if (select)
    {
      ...
      if (! RIB_SYSTEM_ROUTE (select))
        rib_install_kernel (rn, select);
      SET_FLAG (select->flags, KROUTE_FLAG_SELECTED);
      redistribute_add (&rn->p, select);
    }
 ...
 * a non-empty sub-queue with lowest priority. wq is equal to kroute->ribq and data
...
static const u_char meta_queue_map[KROUTE_ROUTE_MAX] = {
  [KROUTE_ROUTE_SYSTEM]  = 4,
  [KROUTE_ROUTE_KERNEL]  = 0,
  [KROUTE_ROUTE_CONNECT] = 0,
  [KROUTE_ROUTE_STATIC]  = 1,
  [KROUTE_ROUTE_RIP]     = 2,
  [KROUTE_ROUTE_RIPNG]   = 2,
  [KROUTE_ROUTE_OSPF]    = 2,
  [KROUTE_ROUTE_OSPF6]   = 2,
  [KROUTE_ROUTE_ISIS]    = 2,
  [KROUTE_ROUTE_BGP]     = 3,
  [KROUTE_ROUTE_HSLS]    = 4,
  [KROUTE_ROUTE_BABEL]   = 2,
};
...
static void
rib_queue_add (struct kroute_t *kroute, struct route_node *rn)
{
   ...
   */
  if (!kroute->ribq->items->count)
    work_queue_add (kroute->ribq, kroute->mq);

  rib_meta_queue_add (kroute->mq, rn);
...
/* initialise kroute rib work queue */
static void
rib_queue_init (struct kroute_t *kroute)
{
  assert (kroute);
  
  if (! (kroute->ribq = work_queue_new (kroute->master, 
  ...
  kroute->ribq->spec.workfunc = &meta_queue_process;
  kroute->ribq->spec.errorfunc = NULL;
  /* XXX: TODO: These should be runtime configurable via vty */
  kroute->ribq->spec.max_retries = 3;
  kroute->ribq->spec.hold = rib_process_hold_time;
  
  if (!(kroute->mq = meta_queue_new ()))
  {
 ...
 * 
 * Refcounting (aka "locking" throughout the GNU Kroute and Bane code):
 *
	      ...
	      u_int32_t metric, u_char distance, safi_t safi)
{
  ...
  if (distance == 0)
	...
	distance = 150;
      ...
      if (type == KROUTE_ROUTE_BGP && CHECK_FLAG (flags, KROUTE_FLAG_IBGP))
	distance = 200;
    }
  ...
  if (type == KROUTE_ROUTE_KERNEL || type == KROUTE_ROUTE_CONNECT)
    for (nexthop = rib->nexthop; nexthop; nexthop = nexthop->next)
...
void rib_lookup_and_dump (struct prefix_ipv4 * p)
{
  ...
  for (rib = rn->info; rib; rib = rib->next)
  {
    ...
    (
      ...
      (CHECK_FLAG (rib->flags, KROUTE_FLAG_SELECTED) ? "selected" : "NOT selected")
    );
 ...
 * route being installed by kroute in FIB already. Take necessary
  ...
  if (rib->distance == 0)
    {
      ...
      /* iBGP distance is 200. */
      if (rib->type == KROUTE_ROUTE_BGP 
	  && CHECK_FLAG (rib->flags, KROUTE_FLAG_IBGP))
	rib->distance = 200;
    }
  ...
  for (same = rn->info; same; same = same->next)
    {
	  ...
	  && same->type != KROUTE_ROUTE_CONNECT)
        break;
    }
  
  /* If this route is kernel route, set FIB flag to the route. */
  if (rib->type == KROUTE_ROUTE_KERNEL || rib->type == KROUTE_ROUTE_CONNECT)
    for (nexthop = rib->nexthop; nexthop; nexthop = nexthop->next)
      SET_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB);
		 ...
		 struct in_addr *gate, unsigned int ifindex, u_int32_t vrf_id, safi_t safi)
{
  ...
  if (IS_KROUTE_DEBUG_KERNEL && gate)
  ...
  for (rib = rn->info; rib; rib = rib->next)
	...
	fib = rib;
      ...
      if (rib->type != type)
	continue;
      if (rib->type == KROUTE_ROUTE_CONNECT && (nexthop = rib->nexthop) &&
	  nexthop->type == NEXTHOP_TYPE_IFINDEX)
	{
  ...
  if (! same)
    {
      if (fib && type == KROUTE_ROUTE_KERNEL)
	{
	  /* Unset flags. */
  ...
  for (rib = rn->info; rib; rib = rib->next)
    {
       ...
       if (rib->type == KROUTE_ROUTE_STATIC && rib->distance == si->distance)
         break;
    }
...
{
  if (type == KROUTE_ROUTE_CONNECT && IN6_IS_ADDR_UNSPECIFIED (&p->prefix)) {
  ...
  if (type == KROUTE_ROUTE_KERNEL && IN6_IS_ADDR_UNSPECIFIED (&p->prefix)
...
rib_weed_table (struct route_table *table)
{
  ...
  if (table)
      ...
      for (rib = rn->info; rib; rib = next)
	{
	  ...
	  if (rib->table != krouted.rtm_table_default &&
...

/* Delete self installed routes after kroute is relaunched.  */
static void
rib_sweep_table (struct route_table *table)
{
  ...
  if (table)
      ...
      for (rib = rn->info; rib; rib = next)
	{
	  ...
	  if (rib->type == KROUTE_ROUTE_KERNEL && 
	      CHECK_FLAG (rib->flags, KROUTE_FLAG_SELFROUTE))
	    {
  ...
  if (table)
    for (rn = route_top (table); rn; rn = route_next (rn))
      for (rib = rn->info; rib; rib = rib->next)
        {
          if (! RIB_SYSTEM_ROUTE (rib)
	      && CHECK_FLAG (rib->flags, KROUTE_FLAG_SELECTED))
            rib_uninstall_kernel (rn, rib);
        }
}
